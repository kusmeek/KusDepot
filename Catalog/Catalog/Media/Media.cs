...
public sealed partial class Catalog
{
    ...
    private async Task<IResult> SearchMedia(MediaQuery? search , IDataConfigs dc , ICatalogDBFactory cdb , HttpContext hc)
    {
        try
        {
            ...
            String? ds = _?.Context.SpanId.ToString(); if(String.IsNullOrEmpty(t)) { Log.Error(SMUnAuth); SetErr(_); ETW.Log.SMError(SMUnAuth); return Results.Unauthorized(); }

            StorageSilo? s = await dc.GetAuthorizedReadSilo(t,dt,ds); if(s is null) { Log.Error(SMUnAuth); SetErr(_); ETW.Log.SMError(SMUnAuth); return Results.Unauthorized(); }
        ...
        catch ( Exception _ ) { Log.Error(_,SMFail); ETW.Log.SMError(_.Message); return Results.Problem(); }
    }
}