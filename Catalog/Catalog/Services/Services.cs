...
public sealed partial class Catalog
{
    ...
    private async Task<IResult> SearchServices(ServiceQuery? search , IDataConfigs dc , ICatalogDBFactory cdb , HttpContext hc)
    {
        try
        {
            ...
            String? ds = _?.Context.SpanId.ToString(); if(String.IsNullOrEmpty(t)) { Log.Error(SSVUnAuth); SetErr(_); ETW.Log.SSVError(SSVUnAuth); return Results.Unauthorized(); }

            StorageSilo? s = await dc.GetAuthorizedReadSilo(t,dt,ds); if(s is null) { Log.Error(SSVUnAuth); SetErr(_); ETW.Log.SSVError(SSVUnAuth); return Results.Unauthorized(); }
        ...
        catch ( Exception _ ) { Log.Error(_,SSVFail); ETW.Log.SSVError(_.Message); return Results.Problem(); }
    }
}