...
public sealed partial class Catalog
{
    ...
    private async Task<IResult> SearchNotes(NoteQuery? search , IDataConfigs dc , ICatalogDBFactory cdb , HttpContext hc)
    {
        try
        {
            ...
            if(String.IsNullOrEmpty(t)) { Log.Error(SNUnAuth); SetErr(_); ETW.Log.SNError(SNUnAuth); return Results.Unauthorized(); }

            if(search is null || search.Notes is null || Equals(search.Notes.Length,0)) { Log.Error(BadArg); SetErr(_); ETW.Log.SNError(BadArg); return Results.BadRequest(); }

            StorageSilo? s = await dc.GetAuthorizedReadSilo(t,dt,ds); if(s is null) { Log.Error(SNUnAuth); SetErr(_); ETW.Log.SNError(SNUnAuth); return Results.Unauthorized(); }
        ...
        catch ( Exception _ ) { Log.Error(_,SNFail); ETW.Log.SNError(_.Message); return Results.Problem(); }
    }
}