...
public sealed partial class Catalog
{
    ...
    private async Task<IResult> SearchElements(ElementQuery? search , IDataConfigs dc , ICatalogDBFactory cdb , HttpContext hc)
    {
        try
        {
            ...
            String? ds = _?.Context.SpanId.ToString(); if(String.IsNullOrEmpty(t)) { Log.Error(SELUnAuth); SetErr(_); ETW.Log.SELError(SELUnAuth); return Results.Unauthorized(); }

            StorageSilo? s = await dc.GetAuthorizedReadSilo(t,dt,ds); if(s is null) { Log.Error(SELUnAuth); SetErr(_); ETW.Log.SELError(SELUnAuth); return Results.Unauthorized(); }
        ...
        catch ( Exception _ ) { Log.Error(_,SELFail); ETW.Log.SELError(_.Message); return Results.Problem(); }
    }
}