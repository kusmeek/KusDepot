...
public sealed partial class Catalog
{
    ...
    private async Task<IResult> SearchCommands(CommandQuery? search , IDataConfigs dc , ICatalogDBFactory cdb , HttpContext hc)
    {
        try
        {
            ...
            String? ds = _?.Context.SpanId.ToString(); if(String.IsNullOrEmpty(t)) { Log.Error(SCUnAuth); SetErr(_); ETW.Log.SCError(SCUnAuth); return Results.Unauthorized(); }

            StorageSilo? s = await dc.GetAuthorizedReadSilo(t,dt,ds); if(s is null) { Log.Error(SCUnAuth); SetErr(_); ETW.Log.SCError(SCUnAuth); return Results.Unauthorized(); }
        ...
        catch ( Exception _ ) { Log.Error(_,SCFail); ETW.Log.SCError(_.Message); return Results.Problem(); }
    }
}