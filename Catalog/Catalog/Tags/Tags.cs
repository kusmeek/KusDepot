...
public sealed partial class Catalog
{
    ...
    private async Task<IResult> SearchTags(TagQuery? search , IDataConfigs dc , ICatalogDBFactory cdb , HttpContext hc)
    {
        try
        {
            ...
            if(String.IsNullOrEmpty(tk)) { Log.Error(STGUnAuth); SetErr(_); ETW.Log.STGError(STGUnAuth); return Results.Unauthorized(); }

            if(search is null || search.Tags is null || Equals(search.Tags.Length,0)) { Log.Error(BadArg); SetErr(_); ETW.Log.STGError(BadArg); return Results.BadRequest(); }

            StorageSilo? s = await dc.GetAuthorizedReadSilo(tk,dt,ds); if(s is null) { Log.Error(STGUnAuth); SetErr(_); ETW.Log.STGError(STGUnAuth); return Results.Unauthorized(); }
        ...
        catch ( Exception _ ) { Log.Error(_,STGFail); ETW.Log.STGError(_.Message); return Results.Problem(); }
    }
}