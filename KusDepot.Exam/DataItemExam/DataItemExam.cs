...
public class DataItemExam
{
    ...
    public async Task SetContentStreamed_GetContentStreamed()
    {
        ...
        Check.That(await item.SetContentStreamed(false)).IsTrue();
    }
    ...
    {
        var item = new DataItemTest();
        Check.That(await item.SignData("field", KeyM)).IsNull();
        Check.That(await item.SignData(null, KeyM)).IsNull();
        Check.That(await item.SignData("field", null)).IsNull();
    }
}