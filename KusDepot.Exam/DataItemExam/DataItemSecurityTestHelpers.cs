...
public static class DataItemSecurityTestHelpers
{
    ...
    public static async Task<String> SetupStreamedContent(DataItem item, Byte[] data)
    {
        String tempFile = Path.GetTempFileName() + ".kdi";
        File.WriteAllBytes(tempFile, data);
        item.SetFILE(tempFile);
        Assert.That(await item.SetContentStreamed(true,(DataItemSecurityContext?)null), Is.True);
        return tempFile;
    }
}
