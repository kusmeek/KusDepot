...
public class DataFactoryExam
{
    ...
    public void CreateDataSetItemAction()
    {
        ...
        {
            ds.SetContent(new DataItem[] { t, b }, (DataItemSecurityContext?)null);
            ds.SetID(id);
    ...
    [Test]
    public void CreateDataStreamItem()
    {
        using var rng = RandomNumberGenerator.Create();
        ...
        var item = DataFactory.CreateDataStreamItem(content, id, "stream", new[] { "note-stream" }, new[] { "tag-stream" }, "STREAM");

        Check.That(item).IsNotNull();
        Check.That(ReferenceEquals(item!.GetContent(),content)).IsTrue();
        Check.That(item.GetID()).IsEqualTo(id);
        Check.That(item.GetName()).IsEqualTo("stream");
    }

    [Test]
    public void CreateDataStreamItemAction()
    {
        using var rng = RandomNumberGenerator.Create();
        ...
        var item = DataFactory.CreateDataStreamItem(s =>
        {
            Check.That(s.SetContent(content)).IsTrue();
            Check.That(s.SetID(id)).IsTrue();
            Check.That(s.SetName("stream-action")).IsTrue();
        });

        Check.That(item).IsNotNull();
        Check.That(ReferenceEquals(item!.GetContent(),content)).IsTrue();
        Check.That(item.GetID()).IsEqualTo(id);
        Check.That(item.GetName()).IsEqualTo("stream-action");
    }

    [Test]
    public async Task CreateDataStreamItemActionAsync()
    {
        using var rng = RandomNumberGenerator.Create();
        ...
        var item = await DataFactory.CreateDataStreamItemAsync(async s =>
        {
            await Task.Yield();
            Check.That(s.SetContent(content)).IsTrue();
            Check.That(s.SetID(id)).IsTrue();
            Check.That(s.SetName("stream-action-async")).IsTrue();
        });

        Check.That(item).IsNotNull();
        Check.That(ReferenceEquals(item!.GetContent(),content)).IsTrue();
        Check.That(item.GetID()).IsEqualTo(id);
        Check.That(item.GetName()).IsEqualTo("stream-action-async");
    }
