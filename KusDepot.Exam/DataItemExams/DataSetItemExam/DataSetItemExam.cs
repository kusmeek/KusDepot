...
public partial class DataSetItemExam
{
    ...
    public void AddDataItems()
    {
        ...
        Check.That(_0.AddDataItems(_, (DataItemSecurityContext?)null)).IsTrue();
    ...
    {
        DataSetItem _0 = new DataSetItem(content:new HashSet<DataItem>(),id:Guid.NewGuid(),name:String.Empty,notes:new HashSet<String>(),tags:new HashSet<String>(),type:DataTypes.SGML);
    ...
    public void EqualsInterface()
    {
        ...
        Check.That(new DataSetItem().Equals(_3)).IsTrue();
    ...
    public async Task GetContent()
    {
        ...
        Check.That(_0.SetContent(_2,(DataItemSecurityContext?)null)).IsTrue();
        var dc1 = await _0.GetDataContent(security:null);
        Check.That(dc1!.DataSet!.First()).IsInstanceOf<GenericItem>();
        Check.That(_0.GetContent()!.First()!).IsInstanceOf<GenericItem>();
        Check.That(_0.RegisterManager(KeyM)).IsTrue();
        Check.That(_0.Lock(KeyM)).IsTrue();
        Check.That(_0.SetContent(_2,(DataItemSecurityContext?)null)).IsFalse();
    ...
    public async Task GetDataContent()
    {
        ...
        Check.That(_i.SetContent(_c,(DataItemSecurityContext?)null)).IsTrue();

        var dc1 = await _i.GetDataContent(security:null);
        ...
        var dc3 = await _i.GetDataContent(DataItemSecurityContextExamHelpers.CreateContext(_i,KeyM!));
        Check.That(dc3).IsNotNull();
        Check.That(dc3!.DataSet).IsNotNull().And.IsEquivalentTo(_c);
    }

    [Test]
    public void GetDataItemsByIDs()
    {
        TextItem _0 = new("Alpha");
        CodeItem _1 = new("Beta");
        BinaryItem _2 = new(RandomNumberGenerator.GetBytes(32));
        DataSetItem _3 = new([_0,_1,_2]);
        HashSet<Guid> _4 = [_0.GetID()!.Value,_2.GetID()!.Value,Guid.NewGuid()];

        HashSet<DataItem>? _5 = _3.GetDataItems(_4,security:null);

        Check.That(_5).IsNotNull();
        Check.That(_5!.Select(i => i.GetID())).IsEquivalentTo(new Guid?[]{_0.GetID(),_2.GetID()});
    }

    [Test]
    public void GetDataItemsEmptyIDs()
    {
        TextItem _0 = new("Alpha");
        CodeItem _1 = new("Beta");
        DataSetItem _2 = new([_0,_1]);

        HashSet<DataItem>? _3 = _2.GetDataItems([],security:null);

        Check.That(_3).IsNotNull();
        Check.That(_3).IsEmpty();
    }

    [Test]
    public void GetDataItemsNullGetsAll()
    {
        TextItem _0 = new("Alpha");
        CodeItem _1 = new("Beta");
        DataSetItem _2 = new([_0,_1]);
        ...
        Check.That(_3).IsNotNull();
        Check.That(_3!).ContainsExactly(_0,_1);
    }

    [Test]
    public async Task GetDataItemsByIDsEncrypted()
    {
        TextItem _0 = new("Alpha");
        CodeItem _1 = new("Beta");
        BinaryItem _2 = new(RandomNumberGenerator.GetBytes(32));
        DataSetItem _3 = new([_0,_1,_2]);
        HashSet<Guid> _4 = [_1.GetID()!.Value,_2.GetID()!.Value,Guid.NewGuid()];

        Check.That(_3.RegisterManager(KeyM)).IsTrue();
        Check.That(await _3.EncryptData(DataItemSecurityContextExamHelpers.CreateContext(_3,KeyM!))).IsTrue();

        HashSet<DataItem>? _5 = _3.GetDataItems(_4,DataItemSecurityContextExamHelpers.CreateContext(_3,KeyM!));

        Check.That(_5).IsNotNull();
        Check.That(_5!.Select(i => i.GetID())).IsEquivalentTo(new Guid?[]{_1.GetID(),_2.GetID()});
    }
    ...
    public async Task GetHashCodeAsync()
    {
        ...
        HashSet<DataItem> seed =
        [
            new TextItem("Alpha"),
            new CodeItem("Console.WriteLine(\"Beta\");"),
            DataItemGenerator.CreateDataSet(3)
        ];

        Check.That(item.SetContent(seed,(DataItemSecurityContext?)null)).IsTrue();

        var syncHash = item.GetHashCode();
        var asyncHash = await item.GetHashCodeAsync();

        Check.That(asyncHash).IsEqualTo(syncHash);

        HashSet<DataItem> additional =
        [
            new BinaryItem(RandomNumberGenerator.GetBytes(256)),
            DataItemGenerator.CreateDataSet(2)
        ];

        Check.That(item.AddDataItems(additional,(DataItemSecurityContext?)null)).IsTrue();
        ...
        Guid? toRemove = item.GetContent()!.First().GetID();
        Check.That(item.RemoveDataItem(toRemove,security:null)).IsTrue();
    ...
    {
        DataSetItem _0 = new DataSetItem();
        HashSet<DataItem> _1 = [new TextItem("Alpha")];

        Check.That(_0.IsMetadataOnly()).IsTrue();
        Check.That(_0.SetContent(_1,(DataItemSecurityContext?)null)).IsTrue();
    ...
    public void RemoveDataItem()
    {
        ...
        Check.That(_0.RemoveDataItem(_.First().GetID(),security:null)).IsTrue();

        Check.That(_0.GetHashCode()).IsNotEqualTo(h);
    }

    [Test]
    public void RemoveDataItems()
    {
        TextItem _0 = new("Alpha");
        CodeItem _1 = new("Beta");
        BinaryItem _2 = new(RandomNumberGenerator.GetBytes(32));
        DataSetItem _3 = new([_0,_1,_2]);
        HashSet<Guid> _4 = [_0.GetID()!.Value,_2.GetID()!.Value];

        Int32 _5 = _3.GetHashCode();

        Check.That(_3.RemoveDataItems(_4,security:null)).IsTrue();
        Check.That(_3.GetDataItems(security:null)).ContainsExactly(_1);
        Check.That(_3.GetHashCode()).IsNotEqualTo(_5);
    }

    [Test]
    public async Task RemoveDataItemsEncrypted()
    {
        TextItem _0 = new("Alpha");
        CodeItem _1 = new("Beta");
        BinaryItem _2 = new(RandomNumberGenerator.GetBytes(32));
        DataSetItem _3 = new([_0,_1,_2]);
        HashSet<Guid> _4 = [_0.GetID()!.Value,_1.GetID()!.Value];

        Check.That(_3.RegisterManager(KeyM)).IsTrue();
        Check.That(await _3.EncryptData(DataItemSecurityContextExamHelpers.CreateContext(_3,KeyM!))).IsTrue();

        Check.That(_3.RemoveDataItems(_4,DataItemSecurityContextExamHelpers.CreateContext(_3,KeyM!))).IsTrue();
        Check.That(_3.GetDataEncrypted()).IsTrue();
        Check.That(_3.GetDataItems(null,DataItemSecurityContextExamHelpers.CreateContext(_3,KeyM!))).ContainsExactly(_2);
    }
    ...
    public async Task SetContentStreamed()
    {
        ...
        Check.That(await _0.SetContentStreamed(true,security:null)).IsTrue();
        ...
        Check.That(SHA512.HashData(d).AsSpan().SequenceEqual(SHA512.HashData(c))).IsTrue();

        Check.That(await _0.SetContentStreamed(false,security:null)).IsTrue();
    ...
    public void SetDataType()
    {
        ...
        Check.That(_0.SetDataType(DataTypes.NFO)).IsTrue();
        Check.That(_0.RegisterManager(KeyM)).IsTrue();
        Check.That(_0.Lock(KeyM)).IsTrue();
        Check.That(_0.GetDataType()).Equals(DataTypes.NFO);
        Check.That(_0.SetDataType(DataTypes.ADMX)).IsFalse();
    }
    ...
    public void Serialize()
    {
        ...
        var _0 = new DataSetItem([new MultiMediaItem(_1)]);

        Byte[] _2 = _0.Serialize();

        DataSetItem? _3 = DataItem.Deserialize<DataSetItem>(_2);

        Check.That(_2.Length).IsStrictlyGreaterThan(0);
        Check.That(_3).IsNotNull();
        Check.That(((DataSetItem)_0).Equals(_3)).IsTrue();
    }
