...
public class BinaryItemSecurityExam
{
    // =====================
    ...
    public async Task EncryptData_SetsAndClears_DataEncryptInfo()
    {
        ...
        RandomNumberGenerator.Create().GetBytes(content);
        var item = new BinaryItem(content);
        var key = item.CreateManagementKey("TestKey");
    ...
    {
        var item = new BinaryItem();
        var initialContent = new Byte[128];
        RandomNumberGenerator.Create().GetBytes(initialContent);
        var key = item.CreateManagementKey("TestKey");
        ...
        var newContent = new Byte[256];
        RandomNumberGenerator.Create().GetBytes(newContent);
    ...
    public async Task SetContent_Encrypted_EmptyContent()
    {
        ...
        Check.That(item.SetContent(Array.Empty<Byte>(), key)).IsTrue();
        Check.That(await item.VerifyData("EncryptedData", key)).IsFalse();
    ...
    public async Task ToFile_FromFile_Roundtrip_Unencrypted()
    {
        ...
        try
        {
            ...
            var loadedItem = BinaryItem.FromFile(file);
    ...
    public async Task WipeData_InMemory_NotEncrypted()
    {
        var content = new Byte[32];
        RandomNumberGenerator.Create().GetBytes(content);
        var item = BinaryItem.FromFile(Path.Join(AppContext.BaseDirectory,"ContainerAssembly.bin"))!;
        var key = item.CreateManagementKey("TestKey");
        Assert.That(await item.SignData("Content", key), Is.Not.Null);
