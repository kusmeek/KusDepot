...
public partial class CodeItemExam
{
    ...
    {
        String _0 = "ExplicitOperatorExam";

        CodeItem? _1 = (CodeItem?)_0;
        CodeItem? _2 = (CodeItem?)null;

        Check.That(_1).IsNotNull();
        Check.That(_1!.GetContent()).IsEqualTo(_0);
    ...
    public void GetDataType()
    {
        ...
        Check.That(_0.SetDataType(DataTypes.C)).IsTrue();
        Check.That(_0.RegisterManager(KeyM)).IsTrue();
        Check.That(_0.Lock(KeyM)).IsTrue();
        Check.That(_0.GetDataType()).Equals(DataTypes.C);
    }
    ...
    public void GetDescriptor()
    {
        ...
        Check.That(_0.SetDataType(DataTypes.KQL)).IsTrue();
        ...
        Check.That(_.DataType).Equals(DataTypes.KQL);
    ...
    public void SetDataType()
    {
        ...
        Check.That(_0.SetDataType(DataTypes.RAZOR)).IsTrue();
        Check.That(_0.RegisterManager(KeyM)).IsTrue();
        Check.That(_0.Lock(KeyM)).IsTrue();
        Check.That(_0.GetDataType()).Equals(DataTypes.RAZOR);
        Check.That(_0.SetDataType(DataTypes.BAT)).IsFalse();
    }
    ...
    {
        var _0 = new CodeItem(TestCaseDataGenerator.GenerateUnicodeString(8192));

        Byte[] _1 = _0.Serialize();

        CodeItem? _2 = DataItem.Deserialize<CodeItem>(_1);

        Check.That(_1.Length).IsStrictlyGreaterThan(0);
        Check.That(_2).IsNotNull();
        Check.That(((CodeItem)_0).Equals(_2)).IsTrue();
    }
    ...
    {
        String text = TestCaseDataGenerator.GenerateUnicodeString(4096);
        CodeItem item = new CodeItem(text);
        ...
        text = TestCaseDataGenerator.GenerateUnicodeString(2048);
        Check.That(item.SetContent(text)).IsTrue();
