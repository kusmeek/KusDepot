...
public partial class KeySetExam
{
    ...
    public void Constructor()
    {
        ...
        Check.That(ks.GetAllKeys()).IsNull();
        Check.That(ks.GetAllKeys(DataItemSecurityContextExamHelpers.CreateContext(ks,owner!))).IsNotNull();
    }
    ...
    {
        var ks = new KeySet();
        Check.That(ks.AddKeys(null)).IsFalse();
        var ok = ks.AddKeys(Array.Empty<SecurityKey>());
    ...
    public void AddKeys_Mixed_ReturnsFalseWhenAnyDuplicate()
    {
        ...
        var ok = ks.AddKeys(new SecurityKey[] { k1, k2, k3 });
    ...
    public void AddKeys_Locked_Gating()
    {
        ...
        Check.That(ks.AddKeys(list)).IsFalse();
        Check.That(ks.GetAllKeys(DataItemSecurityContextExamHelpers.CreateContext(ks,owner!))).IsNull();

        using var wrong = CreateCertificate(Guid.NewGuid(), "wrong");
        var wrongMgr = new ManagerKey(wrong!);
        Check.That(ks.AddKeys(list,DataItemSecurityContextExamHelpers.CreateContext(ks,wrongMgr))).IsFalse();
        Check.That(ks.GetAllKeys(DataItemSecurityContextExamHelpers.CreateContext(ks,owner!))).IsNull();

        Check.That(ks.AddKeys(list,DataItemSecurityContextExamHelpers.CreateContext(ks,owner!))).IsTrue();
        var all = (ks.GetAllKeys(DataItemSecurityContextExamHelpers.CreateContext(ks,owner!)))!;
        Check.That(all.Count).IsEqualTo(2);
    }
    ...
    public void RemoveKeys_Null_And_Empty()
    {
        var ks = new KeySet();
        var keys = CreateUniqueKeys(2);
        Check.That(ks.AddKeys(keys)).IsTrue();
        var okNull = ks.RemoveKeys((IEnumerable<SecurityKey>?)null);
        Check.That(okNull).IsFalse();
        var okEmpty = ks.RemoveKeys(Array.Empty<SecurityKey>());
    ...
    public void RemoveKeys_PartialAndReturnValue()
    {
        ...
        var a = keys[0];
        var b = keys[1];
        Check.That(ks.AddKeys(new SecurityKey[]{a,b})).IsTrue();
        var nonExisting = CreateServiceKey(53, Guid.NewGuid());
        var ok = ks.RemoveKeys(new SecurityKey[]{a,nonExisting});
        Check.That(ok).IsFalse();
        var left = (ks.GetAllKeys())!;
    ...
    public void RemoveKeys_Locked_Gating()
    {
        ...
        using var wrong = CreateCertificate(Guid.NewGuid(), "wrong");
        var wrongMgr = new ManagerKey(wrong!);
        Check.That(ks.RemoveKeys(new SecurityKey[]{a,b})).IsFalse();
        Check.That(ks.RemoveKey(a.ID!.Value.ToString(),DataItemSecurityContextExamHelpers.CreateContext(ks,wrongMgr))).IsFalse();
        Check.That(ks.RemoveKey(a.ID!.Value.ToString(),DataItemSecurityContextExamHelpers.CreateContext(ks,mgr!))).IsTrue();
        var left = (ks.GetAllKeys(DataItemSecurityContextExamHelpers.CreateContext(ks,mgr!)))!;
        Check.That(left.Count).IsEqualTo(1);
        Check.That(left.Values.Any(k => k.Equals(b))).IsTrue();
    }
    ...
    public void DuplicateHandling_EachConcreteType()
    {
        ...
        var okBytes = new OwnerKey(pfx!, id8);
    ...
    public void Remove_EqualInstance_EachConcreteType()
    {
        ...
        foreach (var eq in items) Check.That(ks.RemoveKey(eq.ID!.Value.ToString())).IsTrue();
        Check.That(ks.GetAllKeys()).IsNull();
    }
    ...
    public void Locked_Gating_With_Manager_And_Owner()
    {
        ...
        Check.That(ks.AddKey(k, null, DataItemSecurityContextExamHelpers.CreateContext(ks,manager!))).IsTrue();
        var s2 = CreateServiceKey(42, Guid.NewGuid());
        Check.That(ks.AddKey(s2, null, DataItemSecurityContextExamHelpers.CreateContext(ks,owner!))).IsTrue();

        Check.That(ks.GetAllKeys()).IsNull();
        Check.That(ks.GetAllKeys(DataItemSecurityContextExamHelpers.CreateContext(ks,manager!))).IsNull();
        Check.That(ks.GetAllKeys(DataItemSecurityContextExamHelpers.CreateContext(ks,owner!))).IsNotNull();
    }
    ...
    public void GetAllKeys_Locked_Gating()
    {
        ...
        Check.That(ks.AddKey(sk, null, DataItemSecurityContextExamHelpers.CreateContext(ks,owner!))).IsTrue();

        Check.That(ks.GetAllKeys()).IsNull();

        using var wrongCert = CreateCertificate(Guid.NewGuid(), "Wrong");
        var wrongMgr = new ManagerKey(wrongCert!);
        Check.That(ks.GetAllKeys(DataItemSecurityContextExamHelpers.CreateContext(ks,wrongMgr))).IsNull();

        var result = ks.GetAllKeys(DataItemSecurityContextExamHelpers.CreateContext(ks,owner!));
    ...
    public void GetAllKeys_ReturnedSet_Independent()
    {
        ...
        var r1 = (ks.GetAllKeys())!;
        Check.That(r1.Count).IsEqualTo(2);
        var toRemove = r1.First();
        r1.Remove(toRemove.Key);
        var r2 = (ks.GetAllKeys())!;
        Check.That(r2.Count).IsEqualTo(2);
    }

    [Test]
    public void GetKeys_Filtered_Unlocked_ReturnsClones()
    {
        var ks = new KeySet();
        var a = CreateServiceKey(70, Guid.NewGuid());
        var b = CreateHostKey(71, Guid.NewGuid());

        Check.That(ks.AddKey(a, "Alpha")).IsTrue();
        Check.That(ks.AddKey(b, "Beta")).IsTrue();

        var result = ks.GetKeys(["Alpha"]);

        Check.That(result).IsNotNull();
        Check.That(result!.Count).IsEqualTo(1);
        Check.That(result.ContainsKey("Alpha")).IsTrue();
        Check.That(result["Alpha"].Equals(a)).IsTrue();
        Check.That(ReferenceEquals(result["Alpha"], a)).IsFalse();
    }

    [Test]
    public void GetKeys_Null_GetsAll()
    {
        var ks = new KeySet();
        var a = CreateServiceKey(72, Guid.NewGuid());
        var b = CreateHostKey(73, Guid.NewGuid());

        Check.That(ks.AddKey(a, "Alpha")).IsTrue();
        Check.That(ks.AddKey(b, "Beta")).IsTrue();

        var result = ks.GetKeys();

        Check.That(result).IsNotNull();
        Check.That(result!.Count).IsEqualTo(2);
        Check.That(result.Keys).Contains("Alpha","Beta");
    }
    ...
    public void RemoveKey_Unlocked_Scenarios()
    {
        ...
        Check.That(ks.RemoveKey(sk.ID!.Value.ToString())).IsTrue();
        Check.That(ks.GetAllKeys()).IsNull();

        var sk2 = CreateServiceKey(5, Guid.NewGuid());
        Check.That(ks.AddKey(sk2, null)).IsTrue();
        var equal = CreateServiceKey(5, sk2.ID.Value);
        Check.That(ks.RemoveKey(equal.ID!.Value.ToString())).IsTrue();

        var sk3 = CreateServiceKey(6, Guid.NewGuid());
        Check.That(ks.RemoveKey(sk3.ID!.Value.ToString())).IsFalse();
    }

    [Test]
    public void RemoveKeys_ByName_Unlocked()
    {
        var ks = new KeySet();
        var a = CreateServiceKey(80, Guid.NewGuid());
        var b = CreateHostKey(81, Guid.NewGuid());

        Check.That(ks.AddKey(a, "Alpha")).IsTrue();
        Check.That(ks.AddKey(b, "Beta")).IsTrue();

        var ok = ks.RemoveKeys(["Alpha","Missing"]);

        Check.That(ok).IsFalse();
        var left = ks.GetAllKeys();
        Check.That(left).IsNotNull();
        Check.That(left!.Count).IsEqualTo(1);
        Check.That(left.ContainsKey("Beta")).IsTrue();
    }
    ...
    public void RemoveKey_Locked_Gating_And_LastItem_Nulls_AllKeys()
    {
        ...
        var a = keys[0];
        var b = keys[1];
        Check.That(ks.AddKey(a, null, DataItemSecurityContextExamHelpers.CreateContext(ks,mgr!))).IsTrue();
        Check.That(ks.AddKey(b, null, DataItemSecurityContextExamHelpers.CreateContext(ks,mgr!))).IsTrue();

        using var wrongCert = CreateCertificate(Guid.NewGuid(), "Wrong");
        var wrongMgr = new ManagerKey(wrongCert!);
        Check.That(ks.RemoveKey(a.ID!.Value.ToString())).IsFalse();
        Check.That(ks.RemoveKey(a.ID!.Value.ToString(), DataItemSecurityContextExamHelpers.CreateContext(ks,wrongMgr))).IsFalse();
        ...
        Check.That(ks.RemoveKey(b.ID!.Value.ToString(), DataItemSecurityContextExamHelpers.CreateContext(ks,mgr!))).IsTrue();
        Check.That(ks.GetAllKeys(DataItemSecurityContextExamHelpers.CreateContext(ks,mgr!))).IsNull();
    }
    ...
    public void ToString_Parse_Roundtrip_Unlocked_And_Locked_AllTypes()
    {
        ...
        Check.That(ks2.AddKey(s2a, null, DataItemSecurityContextExamHelpers.CreateContext(ks2,owner!))).IsTrue();
        Check.That(ks2.AddKey(h2a, null, DataItemSecurityContextExamHelpers.CreateContext(ks2,owner!))).IsTrue();
        var str2 = ks2.ToString();
        var parsed2 = KeySet.Parse(str2);
        Check.That(parsed2).IsNotNull();
        Check.That(parsed2!.GetAllKeys()).IsNull();
        var p2 = (parsed2.GetAllKeys(DataItemSecurityContextExamHelpers.CreateContext(parsed2,owner!)))!;
    ...
    public async Task GenericItem_Stores_Encrypted_Roundtrip_And_Unlocks_Embedded_KeySet()
    {
        ...
        Check.That(await gi.EncryptData(DataItemSecurityContextExamHelpers.CreateContext(gi,owner!))).IsTrue();
        ...
        var gi2 = round!;
        Check.That(await gi2.DecryptData(DataItemSecurityContextExamHelpers.CreateContext(gi2,owner!))).IsTrue();
        ...
        Check.That(embedded!.GetAllKeys()).IsNull();
        var got = embedded.GetAllKeys(DataItemSecurityContextExamHelpers.CreateContext(embedded,owner!));
    ...
    public void IsMetadataOnly()
    {
        ...
        Check.That(ks.IsMetadataOnly()).IsTrue();
        Check.That(ks.AddKey(CreateServiceKey(99,Guid.NewGuid()),null)).IsTrue();
        Check.That(ks.IsMetadataOnly()).IsFalse();
    }
    ...
    public void Serialize_Roundtrip()
    {
        ...
        var ks = new KeySet(items);
        var bytes = ks.Serialize();
        var roundtrip = DataItem.Deserialize<KeySet>(bytes);

        Check.That(bytes.Length).IsStrictlyGreaterThan(0);
        Check.That(roundtrip).IsNotNull();
        Check.That(roundtrip!.GetID()).IsEqualTo(((KeySet)ks).GetID());
    }
    ...
    public void GetKey_Locked_Gating()
    {
        ...
        var owner = ks.CreateOwnerKey("owner");
        Check.That(ks.Lock(owner)).IsTrue();
        Check.That(ks.AddKey(key, null, DataItemSecurityContextExamHelpers.CreateContext(ks,owner!))).IsTrue();

        Check.That(ks.GetKey(key.ID!.Value.ToString())).IsNull();
        
        using var wrongCert = CreateCertificate(Guid.NewGuid(), "Wrong");
        var wrongMgr = new ManagerKey(wrongCert!);
        Check.That(ks.GetKey(key.ID!.Value.ToString(), DataItemSecurityContextExamHelpers.CreateContext(ks,wrongMgr))).IsNull();

        var retrieved = ks.GetKey(key.ID!.Value.ToString(), DataItemSecurityContextExamHelpers.CreateContext(ks,owner!));
        Check.That(retrieved).IsNotNull();
        Check.That(retrieved).IsEqualTo(key);
    }
    ...
    public async Task AddKey_And_GetKey_ByName_When_Locked_And_Encrypted()
    {
        ...
        Check.That(owner).IsNotNull();
        Check.That(ks.Lock(owner)).IsTrue();
        Check.That(await ks.EncryptData(DataItemSecurityContextExamHelpers.CreateContext(ks,owner!))).IsTrue();
        ...
        foreach (var key in keys)
        {
            ...
            Check.That(ks.AddKey(key, friendlyName, DataItemSecurityContextExamHelpers.CreateContext(ks,owner!))).IsTrue();
        }

        foreach (var key in keys)
        {
            var friendlyName = $"friendly-{key.GetType().Name}";
            var retrieved = ks.GetKey(friendlyName, DataItemSecurityContextExamHelpers.CreateContext(ks,owner!));
            Check.That(retrieved).IsNotNull();
            Check.That(retrieved).IsEqualTo(key);
    ...
    {
        var initialKeys = CreateUniqueKeys(4);
        var ks = new KeySet(initialKeys);

        var syncHash = ks.GetHashCode();
        var asyncHash = await ks.GetHashCodeAsync();

        Check.That(asyncHash).IsEqualTo(syncHash);

        var owner = ks.CreateOwnerKey("hash-owner");
        Check.That(owner).IsNotNull();
        Check.That(ks.Lock(owner)).IsTrue();

        var newKey = CreateServiceKey(90, Guid.NewGuid());
        Check.That(ks.AddKey(newKey, "friendly-service", DataItemSecurityContextExamHelpers.CreateContext(ks,owner!))).IsTrue();

        syncHash = ks.GetHashCode();
        asyncHash = await ks.GetHashCodeAsync();
