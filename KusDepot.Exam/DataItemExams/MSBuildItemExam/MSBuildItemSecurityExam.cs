...
public class MSBuildItemSecurityExam
{
    // =====================
    ...
    public async Task CheckDataHash_StreamedContent_Valid()
    {
        ...
        var file = await DataItemSecurityTestHelpers.SetupStreamedContent(item, data);
        try
        {
            Check.That(await item.CheckDataHash()).IsTrue();
        }
    ...
    {
        var item = new MSBuildItem("test content");
        var key = item.CreateManagementKey("TestKey");
    ...
    {
        var item = new MSBuildItem("locked content");
        var key = item.CreateManagementKey("TestKey");
    ...
    {
        var item = new MSBuildItem("plain content");
        var key = item.CreateManagementKey("TestKey");
        Check.That(await item.DecryptData(key)).IsFalse();
    ...
    public async Task EncryptData_StreamedContent_MissingFile()
    {
        ...
        item.SetFILE(file);
        Check.That(await item.SetContentStreamed(true)).IsFalse();
        Check.That(await item.EncryptData(key, false)).IsFalse();
        Check.That(item.GetDataEncryptInfo()).IsNull();
    }
    ...
    {
        var item = new MSBuildItem("double encrypt");
        var key = item.CreateManagementKey("TestKey");
        Check.That(await item.EncryptData(key, false)).IsTrue();
        Check.That(await item.EncryptData(key, false)).IsFalse();
    }
    ...
    {
        var item = new MSBuildItem("double decrypt");
        var key = item.CreateManagementKey("TestKey");
        Check.That(await item.EncryptData(key, false)).IsTrue();
        Check.That(await item.DecryptData(key)).IsTrue();
        Check.That(await item.DecryptData(key)).IsFalse();
    ...
    {
        var item = new MSBuildItem("wrong key");
        var key = item.CreateManagementKey("TestKey");
    ...
    {
        var item = new MSBuildItem("tampered");
        var key = item.CreateManagementKey("TestKey");
        Check.That(await item.EncryptData(key, false)).IsTrue();
        DataItemSecurityTestHelpers.TamperEncryptedData(item);
        Check.That(await item.DecryptData(key)).IsFalse();
    ...
    {
        var item = new MSBuildItem("Initial Content");
        var key = item.CreateManagementKey("TestKey");
        var requirements = new HashSet<DataItem> { new TextItem("Requirement 1") };

        Check.That(await item.EncryptData(key, true)).IsTrue();
        Check.That(item.GetRequirements()).IsNull();

        Check.That(item.SetRequirements(requirements, key)).IsTrue();
        Check.That(item.GetRequirements()).IsNull();
        Check.That(await item.VerifyData("EncryptedData", key)).IsTrue();
    ...
    public async Task SetRequirements_Encrypted_NoKey()
    {
        ...
        Check.That(await item.EncryptData(key, true)).IsTrue();
        Check.That(item.SetRequirements(requirements, null)).IsFalse();
    }
    ...
    public async Task SetSequence_Encrypted_Roundtrip()
    {
        var item = new MSBuildItem("Initial Content");
        var key = item.CreateManagementKey("TestKey");
        var sequence = new SortedList<Int32, MSBuildItem> { { 0, new MSBuildItem("Sequence 1") } };

        Check.That(await item.EncryptData(key, true)).IsTrue();
        Check.That(item.GetSequence()).IsNull();

        Check.That(item.SetSequence(sequence, key)).IsTrue();
        Check.That(item.GetSequence()).IsNull();
        Check.That(await item.VerifyData("EncryptedData", key)).IsTrue();
    ...
    public async Task SetSequence_Encrypted_NoKey()
    {
        ...
        Check.That(await item.EncryptData(key, true)).IsTrue();
        Check.That(item.SetSequence(sequence, null)).IsFalse();
    }
    ...
    public async Task SignData_And_VerifyData_ValidSignature()
    {
        ...
        var sigKey = await item.SignData("Content", key);
    ...
    public async Task SignData_And_VerifyData_MultipleSignatures_UsesMostRecent()
    {
        ...
        var content = TestCaseDataGenerator.GenerateUnicodeString(1024);
        Assert.That(item.SetContent(content), Is.True);
        var sigKey1 = await item.SignData("Content", key);
        Thread.Sleep(10);
        var sigKey2 = await item.SignData("Content", key);
    ...
    public async Task SignData_And_VerifyData_InvalidOrMissingHash()
    {
        var item = new MSBuildItem();
        var key = item.CreateManagementKey("TestKey");
        Assert.That(await item.SignData("Content", key), Is.Null);
        Assert.That(await item.VerifyData("Content", key), Is.False);
    }
    ...
    public async Task SignData_And_VerifyData_NullKeyOrField()
    {
        ...
        var content = TestCaseDataGenerator.GenerateUnicodeString(1024);
        Assert.That(item.SetContent(content), Is.True);
        Assert.That(await item.SignData(null, key), Is.Null);
        Assert.That(await item.SignData("Content", null), Is.Null);
    ...
    public async Task SignData_And_VerifyData_MultiCertificate_IndependentSignatures()
    {
        ...
        var sigKey2 = await item.SignData("EncryptedData", key2);
        ...
        var sigContentKey2 = await item.SignData("Content", key2);
    ...
    public async Task SignData_And_VerifyData_HashCode_ValidSignature()
    {
        ...
        var content = TestCaseDataGenerator.GenerateUnicodeString(1024);
        Assert.That(item.SetContent(content), Is.True);
        var sigKey = await item.SignData("HashCode", key);
        Assert.That(sigKey, Is.Not.Null.And.Not.Empty);
    ...
    {
        var content = "<Project Sdk=\"Microsoft.NET.Sdk\"></Project>";
        var item = new MSBuildItem(content);
        var key = item.CreateManagementKey("FileRoundtripKey");
        var file = Path.GetTempFileName() + ".kdi";

        try
        {
            Assert.That(await item.SignData("Content", key), Is.Not.Null);
            Assert.That(await item.VerifyData("Content", key), Is.True);
            Assert.That(item.ToFile(file), Is.True);

            var loadedItem = MSBuildItem.FromFile(file);
    ...
    {
        var requirementItem = new TextItem("requirement");
        var sequenceItem = new MSBuildItem("sequence step");
        var item = new MSBuildItem("content");
        item.SetRequirements(new[] { requirementItem });
        item.SetSequence(new Dictionary<Int32, MSBuildItem> { { 1, sequenceItem } });
        
        var key = item.CreateManagementKey("TestKey");
        Assert.That(await item.SignData("Content", key), Is.Not.Null);

        Assert.That(await item.WipeData(key), Is.True);
    ...
    {
        var item = new MSBuildItem("some content");
        var key = item.CreateManagementKey("TestKey");
        Assert.That(await item.EncryptData(key, true), Is.True);
    ...
    public async Task WipeData_Locked_InvalidKey()
    {
        ...
        Assert.That(await item.WipeData(invalidKey), Is.False);
