...
public class TextItemSecurityExam
{
    ...
    {
        var item = new TextItem("some content");
        var key = item.CreateManagementKey("TestKey");
    ...
    public async Task SignData_And_VerifyData_MultiCertificate_IndependentSignatures()
    {
        ...
        Assert.That(item.EncryptData(key1, true).Result, Is.True);
        ...
        Assert.That(item.DecryptData(key1).Result, Is.True);
    ...
    public async Task MaskData_InMemory_NotEncrypted()
    {
        ...
        Assert.That((await item.GetDataContent())!.Text, Is.EqualTo(content));
        ...
        Assert.That((await item.GetDataContent(key))!.Text, Is.EqualTo(content));
    ...
    {
        var content = "zero-inmem-plain-" + Guid.NewGuid();
        var item = new TextItem(content);
        var key = item.CreateManagementKey("TestKey");
        Assert.That(item.SignData("Content", key), Is.Not.Null);
    ...
    {
        var content = "zero-inmem-encrypted-" + Guid.NewGuid();
        var item = new TextItem(content);
        var key = item.CreateManagementKey("TestKey");
        Assert.That(await item.EncryptData(key, true), Is.True);
    ...
    {
        var content = "zero-locked-invalid-" + Guid.NewGuid();
        var item = new TextItem(content);
    ...
    {
        var content = "zero-locked-valid-" + Guid.NewGuid();
        var item = new TextItem(content);
        var key = item.CreateManagementKey("TestKey");
