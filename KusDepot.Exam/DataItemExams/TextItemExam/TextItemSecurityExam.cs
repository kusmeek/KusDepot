...
public class TextItemSecurityExam
{
    // =====================
    ...
    [Test]
    public static async Task VerifyData_EncryptedData_StreamedContent_ValidSignature()
    {
        var item = new TextItem();
        var data = new Byte[1024];
        RandomNumberGenerator.Create().GetBytes(data);
    ...
    [Test]
    public static async Task VerifyData_EncryptedData_StreamedContent_Tampered()
    {
        var item = new TextItem();
        var data = new Byte[1024];
        RandomNumberGenerator.Create().GetBytes(data);
    ...
    {
        var item = new TextItem("test content");
        var key = item.CreateManagementKey("TestKey");
    ...
    {
        var item = new TextItem("locked content");
        var key = item.CreateManagementKey("TestKey");
    ...
    {
        var item = new TextItem("plain content");
        var key = item.CreateManagementKey("TestKey");
        Check.That(await item.DecryptData(key)).IsFalse();
    ...
    {
        var item = new TextItem("double encrypt");
        var key = item.CreateManagementKey("TestKey");
        Check.That(await item.EncryptData(key, false)).IsTrue();
        Check.That(await item.EncryptData(key, false)).IsFalse();
    }
    ...
    {
        var item = new TextItem("double decrypt");
        var key = item.CreateManagementKey("TestKey");
        Check.That(await item.EncryptData(key, false)).IsTrue();
        Check.That(await item.DecryptData(key)).IsTrue();
        Check.That(await item.DecryptData(key)).IsFalse();
    ...
    {
        var item = new TextItem("wrong key");
        var key = item.CreateManagementKey("TestKey");
    ...
    {
        var item = new TextItem("tampered");
        var key = item.CreateManagementKey("TestKey");
        Check.That(await item.EncryptData(key, false)).IsTrue();
        DataItemSecurityTestHelpers.TamperEncryptedData(item);
        Check.That(await item.DecryptData(key)).IsFalse();
    ...
    {
        var content = "unencrypted-file-roundtrip-" + Guid.NewGuid();
        var item = new TextItem(content);
        ...
        try
        {
            ...
            var loadedItem = TextItem.FromFile(file);
    ...
    {
        var content = "encrypted-file-roundtrip-" + Guid.NewGuid();
        var item = new TextItem(content);
        var key = item.CreateManagementKey("FileRoundtripKey");
        var file = Path.GetTempFileName() + ".kdi";
    ...
    {
        var content = "wipe-inmem-plain-" + Guid.NewGuid();
        var item = new TextItem(content);
        var key = item.CreateManagementKey("TestKey");
        Assert.That(await item.SignData("Content", key), Is.Not.Null);
    ...
    {
        var content = "wipe-inmem-encrypted-" + Guid.NewGuid();
        var item = new TextItem(content);
        var key = item.CreateManagementKey("TestKey");
        Assert.That(await item.EncryptData(key, true), Is.True);
    ...
    {
        var content = "wipe-locked-invalid-" + Guid.NewGuid();
        var item = new TextItem(content);
    ...
    {
        var content = "wipe-locked-valid-" + Guid.NewGuid();
        var item = new TextItem(content);
        var key = item.CreateManagementKey("TestKey");
