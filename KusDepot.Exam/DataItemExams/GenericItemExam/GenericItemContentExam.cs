...
public class GenericItemContentExam
{
    ...
    public async Task GetHashCodeAsync()
    {
        ...
        {
            Int32 syncHash  = gi.GetHashCode();
            Int32 asyncHash = await gi.GetHashCodeAsync().ConfigureAwait(false);
    ...
    [Test]
    public void Serialization_Roundtrip_ToolValue_Content()
    {
        ...
        var gi = new GenericItem(new Object[] { "toolvalue", payload });
        var s = gi.ToString();
        var back = GenericItem.Parse(s);
        ...
        var content = back!.GetContent();
        Check.That(content).IsNotNull();
        Check.That(content![0]).IsEqualTo("toolvalue");
        Check.That(content[1]).IsInstanceOf<ToolValue>();

        var restored = (ToolValue)content[1];
        Check.That(restored.Mode).IsEqualTo(ToolValueMode.Parse);
        Check.That(restored.Type).IsEqualTo(typeof(Guid).FullName);
        Check.That(restored.Data).IsEqualTo("11111111-1111-1111-1111-111111111111");
    }

    [Test]
    public void Serialization_Roundtrip_ToolDataHierarchy_Content()
    {
        ...
        var gi = new GenericItem(new Object[] { tooldata, toolinput, tooloutput });
        var s = gi.ToString();
        var back = GenericItem.Parse(s);
        ...
        var content = back!.GetContent();
        Check.That(content).IsNotNull();
        Check.That(content!.Count).IsEqualTo(3);

        Check.That(content[0]).IsInstanceOf<ToolData>();
        Check.That(content[1]).IsInstanceOf<ToolInput>();
        Check.That(content[2]).IsInstanceOf<ToolOutput>();

        var restoredData = (ToolData)content[0];
        var restoredInput = (ToolInput)content[1];
        var restoredOutput = (ToolOutput)content[2];

        Check.That(restoredData.Data).IsInstanceOf<Version>();
        Check.That(((Version)restoredData.Data!).ToString()).IsEqualTo("1.2.3.4");

        Check.That(restoredInput.Data).IsInstanceOf<Uri>();
        Check.That(restoredInput.Data!.ToString()).IsEqualTo("https://example.org/input");

        Check.That(restoredOutput.ID).IsEqualTo(id);
        Check.That(restoredOutput.Data).IsInstanceOf<DateTimeOffset>();
        Check.That((DateTimeOffset)restoredOutput.Data!).IsEqualTo(stamp);
    }
}