...
public partial class UtilityExam
{
    ...
    [Test]
    public async Task WipeMemory_Stream()
    {
        Byte[] data = RandomBytes(4096);
        using MemoryStream ms = new();
        ms.Write(data,0,data.Length);
        Int64 originalPosition = ms.Position;

        Check.That(await WipeMemoryAsync(ms,CancellationToken.None)).IsTrue();
        Check.That(ms.Position).IsEqualTo(originalPosition);

        Byte[] cleared = new Byte[data.Length];
        ms.Seek(0,SeekOrigin.Begin);
        Int32 read = ms.Read(cleared,0,cleared.Length);

        Check.That(read).IsEqualTo(data.Length);
        Check.That(cleared.All(b => b == 0)).IsTrue();
    }

    [Test]
    public async Task WipeMemory_File()
    {
        ...
        try
        {
            ...
            Check.That(before.SequenceEqual(data)).IsTrue();

            Check.That(await WipeMemoryAsync(path)).IsTrue();

            Byte[] after = await File.ReadAllBytesAsync(path);

            Check.That(after.Length).IsEqualTo(before.Length);
            Check.That(after.SequenceEqual(before)).IsFalse();
            Check.That(after.All(b => b == 0)).IsTrue();
        }
        finally
        {
    ...
    [Test]
    public void WipeMemory_Stream_Sync()
    {
        ...
        Check.That(WipeMemory(ms)).IsTrue();
        Check.That(ms.Position).IsEqualTo(originalPosition);
    ...
    [Test]
    public void WipeMemory_File_Sync()
    {
        String path = Path.GetTempFileName();
        ...
        try
        {
            ...
            Check.That(WipeMemory(path)).IsTrue();
    ...
    [Test]
    public void ZeroMemory_File()
    {
        String path = Path.GetTempFileName() + ".tmp";
        ...
        try
        {
            ...
            Check.That(ZeroMemoryFile(path)).IsTrue();
    ...
    [Test]
    public async Task ZeroMemory_File_Async()
    {
        String path = Path.GetTempFileName() + ".tmp";
        ...
        try
        {
            ...
            Check.That(await ZeroMemoryFileAsync(path)).IsTrue();
    ...
    [Test]
    public async Task ZeroMemory_Stream_Async()
    {
        Byte[] data = RandomBytes(1536);
        using MemoryStream ms = new();
        ms.Write(data,0,data.Length);
        Int64 originalPosition = ms.Position;

        Check.That(await ZeroMemoryAsync(ms,CancellationToken.None)).IsTrue();
