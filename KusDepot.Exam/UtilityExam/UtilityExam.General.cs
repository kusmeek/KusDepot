...
public partial class UtilityExam
{
    ...
    [Test]
    public void GetFileSize_Basic()
    {
        String path = Path.GetTempFileName() + ".bin";
        Int32 length = RandomNumberGenerator.GetInt32(0,4097);
        ...
        try
        {
            ...
            Check.That(size).IsNotNull();
            Check.That(size!.Value).IsEqualTo(data.LongLength);
            Check.That(GetFileSize(null)).IsNull();
            Check.That(GetFileSize(String.Empty)).IsNull();
        }
        finally
        {
            if(File.Exists(path)) { File.Delete(path); }
