...
public class GeneralDataEnvelopeExam
{
    ...
    public void SingleChunk_RoundTrip_Default()
    {
        ...
        Byte[]? env = CryptoCore.Encrypt(plain, cert, chunksizepower: ChunkPower);
    ...
    public void FooterMode_RoundTrip()
    {
        ...
        Byte[]? env = CryptoCore.Encrypt(plain, cert, includeoriginallength: false, chunksizepower: ChunkPower);
        Assert.That(env, Is.Not.Null);
        Byte[]? dec = CryptoCore.Decrypt(env, cert);
        Assert.That(dec, Is.Not.Null);
        Assert.That(dec, Is.EquivalentTo(plain));
    ...
    public void FooterMode_TruncationDetected()
    {
        ...
        Byte[]? dec = CryptoCore.Decrypt(truncated, cert);
    ...
    public void OriginalLength_TruncationDetected()
    {
        ...
        Byte[]? env = CryptoCore.Encrypt(plain, cert, includeoriginallength: true, chunksizepower: ChunkPower);
        Assume.That(env, Is.Not.Null);
    ...
    public void AADHash_Binding_SuccessAndFailure()
    {
        ...
        Byte[]? env = CryptoCore.Encrypt(plain, cert, aad, includeaadhash: true, includeoriginallength: true, chunksizepower: ChunkPower);
        Assert.That(env, Is.Not.Null);
        Byte[]? dec1 = CryptoCore.Decrypt(env, cert, aad);
        Assert.That(dec1, Is.Not.Null);
        Assert.That(dec1, Is.EquivalentTo(plain));
        Byte[] wrong = Encoding.UTF8.GetBytes("wrong-context");
        Byte[]? dec2 = CryptoCore.Decrypt(env, cert, wrong);
        Assert.That(dec2, Is.Null);
        Byte[]? envDefault = CryptoCore.Encrypt(plain, cert, aad, chunksizepower: ChunkPower);
        Assert.That(envDefault, Is.Not.Null);
        Byte[]? decDefaultGood = CryptoCore.Decrypt(envDefault, cert, aad);
        Assert.That(decDefaultGood, Is.Not.Null);
        Assert.That(decDefaultGood, Is.EquivalentTo(plain));
        Byte[]? decDefaultBad = CryptoCore.Decrypt(envDefault, cert, wrong);
    ...
    public void AAD_NotBound_Ignored()
    {
        ...
        Byte[]? env = CryptoCore.Encrypt(plain, cert, aad1, includeaadhash: false, includeoriginallength: true, chunksizepower: ChunkPower);
        Assert.That(env, Is.Not.Null);
        Byte[] aad2 = Encoding.UTF8.GetBytes("different-aad");
        Byte[]? dec = CryptoCore.Decrypt(env, cert, aad2);
        Assert.That(dec, Is.Not.Null);
        Assert.That(dec, Is.EquivalentTo(plain));
    }
    ...
    public void Tamper_Reorder_Chunks_Fails()
    {
        ...
        try
        {
            dec = CryptoCore.Decrypt(tampered, cert);
        }
        catch (Exception ex)
        {
            caught = ex is AuthenticationTagMismatchException;
    ...
    public async Task Async_RoundTrip_Buffer()
    {
        ...
        Byte[]? env = await CryptoCore.EncryptAsync(plain, cert, chunksizepower: ChunkPower);
        Assert.That(env, Is.Not.Null);
    ...
    public async Task Async_Stream_RoundTrip()
    {
        ...
        Assert.That(await CryptoCore.EncryptAsync(input, encrypted, cert, chunksizepower: ChunkPower), Is.True);
    ...
    public async Task Async_Cancelled_Encrypt()
    {
        ...
        Boolean ok = await CryptoCore.EncryptAsync(input, output, cert, chunksizepower: ChunkPower, cancel:cts.Token);
    ...
    public void Span_RoundTrip()
    {
        ...
        Assert.That(CryptoCore.Encrypt(plain, env, cert, out Int32 written), Is.True);
        Assert.That(written, Is.GreaterThan(0));

        Span<Byte> dec = stackalloc Byte[plain.Length];
        Assert.That(CryptoCore.Decrypt(env[..written], dec, cert, out Int32 decwritten), Is.True);
    ...
    public void Span_Interop_EncryptSpan_DecryptArray()
    {
        ...
        Assert.That(CryptoCore.Encrypt(plain, envSpan, cert, out Int32 written), Is.True);

        Byte[]? dec = CryptoCore.Decrypt(envSpan[..written].ToArray(), cert);
        Assert.That(dec, Is.Not.Null);
    ...
    public void Span_Interop_EncryptArray_DecryptSpan()
    {
        ...
        Span<Byte> dec = stackalloc Byte[plain.Length];
        Assert.That(CryptoCore.Decrypt(env, dec, cert, out Int32 written), Is.True);
    ...
    public void Span_AADHash_Binding()
    {
        ...
        Assert.That(CryptoCore.Encrypt(plain, env, cert, out Int32 written, aad, includeaadhash: true), Is.True);

        Span<Byte> dec = stackalloc Byte[plain.Length];
        Assert.That(CryptoCore.Decrypt(env[..written], dec, cert, out Int32 decwritten, aad), Is.True);
        Assert.That(decwritten, Is.EqualTo(plain.Length));
        Assert.That(dec.SequenceEqual(plain), Is.True);

        ReadOnlySpan<Byte> wrongAad = Encoding.UTF8.GetBytes("wrong");
        Assert.That(CryptoCore.Decrypt(env[..written], dec, cert, out _, wrongAad), Is.False);
    }
    ...
    public void Span_OutputTooSmall_Fails()
    {
        ...
        Assert.That(CryptoCore.Encrypt(plain, env, cert, out _), Is.False);

        Byte[]? validEnv = CryptoCore.Encrypt(plain.ToArray(), cert);
        Assume.That(validEnv, Is.Not.Null);
        Span<Byte> dec = stackalloc Byte[plain.Length - 1];
        Assert.That(CryptoCore.Decrypt(validEnv, dec, cert, out _), Is.False);
    }
    ...
    public void Span_Tamper_Fails()
    {
        ...
        Span<Byte> dec = stackalloc Byte[plain.Length];
        Assert.That(CryptoCore.Decrypt(env[..written], dec, cert, out _), Is.False);
    }
    ...
    public void Span_Interop_EncryptSpan_DecryptStream()
    {
        ...
        Assert.That(CryptoCore.Decrypt(inputStream, outputStream, cert), Is.True);
    ...
    public void Span_Interop_EncryptStream_DecryptSpan()
    {
        ...
        Assert.That(CryptoCore.Encrypt(inputStream, encryptedStream, cert, chunksizepower: ChunkPower), Is.True);
    ...
    public void TinyPayloads_VariousModes(Int32 size, Boolean includeOrig, Boolean includeAadHash)
    {
        ...
        Byte[]? env = CryptoCore.Encrypt(pt, cert, aad, includeAadHash, includeOrig, ChunkPower);
    ...
    public void LargeAAD_HashBindingAndUnbound()
    {
        ...
        Byte[]? envBound = CryptoCore.Encrypt(pt, cert, largeAad, includeaadhash: true, includeoriginallength: true, chunksizepower: ChunkPower);
        Assert.That(envBound, Is.Not.Null);
        Byte[]? decOk = CryptoCore.Decrypt(envBound, cert, largeAad);
        Assert.That(decOk, Is.Not.Null);
        Assert.That(decOk, Is.EquivalentTo(pt));
        Byte[] wrong = RandomBytes(2048);
        Byte[]? decFail = CryptoCore.Decrypt(envBound, cert, wrong);
        Assert.That(decFail, Is.Null, "Different AAD must fail when hashed");
        Byte[]? envUnbound = CryptoCore.Encrypt(pt, cert, largeAad, includeaadhash: false, includeoriginallength: true, chunksizepower: ChunkPower);
        Assert.That(envUnbound, Is.Not.Null);
        Byte[] other = RandomBytes(2048);
        Byte[]? decIgnore = CryptoCore.Decrypt(envUnbound, cert, other);
    ...
    public void FooterCorruption_MarkerAndCount()
    {
        ...
        Byte[]? env = CryptoCore.Encrypt(pt, cert, aad: null, includeaadhash: false, includeoriginallength: false, chunksizepower: ChunkPower);
        Assert.That(env, Is.Not.Null);
        Byte[] corruptMarker = (Byte[])env!.Clone();
        if(corruptMarker.Length < 5) Assert.Inconclusive("Envelope too small");
        corruptMarker[^5] ^= 0x0F;
        Assert.That(CryptoCore.Decrypt(corruptMarker, cert), Is.Null, "Corrupt footer marker should fail");
        ...
        Assert.That(CryptoCore.Decrypt(corruptCount, cert), Is.Null, "Footer chunk count mismatch should fail");
    }
    ...
    public void Tamper_TagByte_Fails()
    {
        using var cert = CreateEphemeralCert();
        Byte[] plain = RandomBytes(50000);
        Byte[]? env = CryptoCore.Encrypt(plain, cert, aad: null, includeaadhash: false, includeoriginallength: true, chunksizepower: ChunkPower);
        Assume.That(env, Is.Not.Null);
        Int32 headerLen = ParseHeaderLength(env!);
    ...
    public void Tamper_TruncateAtTagBoundary_Fails()
    {
        ...
        Assert.That(CryptoCore.Decrypt(truncated, cert), Is.Null);
    }
    ...
    public void AAD_Binding_EnvelopesDiffer_ForDifferentAAD()
    {
        ...
        Byte[]? env1 = CryptoCore.Encrypt(pt, cert, aad1, includeaadhash: true, includeoriginallength: true, chunksizepower: ChunkPower);
        Byte[]? env2 = CryptoCore.Encrypt(pt, cert, aad2, includeaadhash: true, includeoriginallength: true, chunksizepower: ChunkPower);
        ...
        Assert.That(CryptoCore.Decrypt(env1, cert, aad1), Is.EqualTo(pt));
        Assert.That(CryptoCore.Decrypt(env2, cert, aad2), Is.EqualTo(pt));
    }
    ...
    public void LargePayload_StreamRoundTrip()
    {
        ...
        using var encrypted = new MemoryStream();
        Assert.That(CryptoCore.Encrypt(input, encrypted, cert, aad: null, includeaadhash: false, includeoriginallength: true, chunksizepower: 20), Is.True);
        encrypted.Position = 0;
        using var output = new MemoryStream();
        Assert.That(CryptoCore.Decrypt(encrypted, output, cert), Is.True);
    ...
    public void ZeroLength_FooterMode_RoundTrip()
    {
        using var cert = CreateEphemeralCert();
        Byte[] plain = Array.Empty<Byte>();
        Byte[]? env = CryptoCore.Encrypt(plain, cert, aad : null, includeaadhash : false, includeoriginallength : false, chunksizepower : ChunkPower);
        Assert.That(env, Is.Not.Null);
        Byte[]? dec = CryptoCore.Decrypt(env, cert);
        Assert.That(dec, Is.Not.Null);
        Assert.That(dec!.Length, Is.EqualTo(0));
    ...
    public void ChunkSizeVariability_Boundaries(Int32 power, Int32 size)
    {
        using var cert = CreateEphemeralCert();
        Byte[] pt = RandomBytes(size);
        Byte[]? env = CryptoCore.Encrypt(pt, cert, aad: null, includeaadhash: false, includeoriginallength: true, chunksizepower: power);
    ...
    public void RSA_DifferentPrivateKey_Fails()
    {
        ...
        Byte[]? env = CryptoCore.Encrypt(pt, certEnc, chunksizepower: ChunkPower);
        Assert.That(env, Is.Not.Null);
        Byte[]? decWrong = null;
        Boolean caught = false;
        try
        {
            decWrong = CryptoCore.Decrypt(env, certWrong);
        }
        catch (Exception ex)
        {
        ...
        Assert.That(CryptoCore.Decrypt(env, certEnc), Is.EqualTo(pt));
    }
    ...
    public void RSA_LargeKeySizes_Succeed(Int32 keySize)
    {
        ...
        Byte[] pt = RandomBytes(50000);
        Byte[]? env = CryptoCore.Encrypt(pt, cert, aad : null, includeaadhash : false, includeoriginallength : true, chunksizepower : ChunkPower);
        Assert.That(env, Is.Not.Null, $"Encrypt failed for keySize {keySize}");
        Assert.That(CryptoCore.Decrypt(env, cert), Is.EqualTo(pt), $"Decrypt failed for keySize {keySize}");
    }
    ...
    public void HeaderCorrupt_ChunkSizePower_OutOfRange_Fails()
    {
        ...
        Byte[]? env = CryptoCore.Encrypt(pt, cert, aad : null, includeaadhash : false, includeoriginallength : true, chunksizepower : 16);
        Assert.That(env, Is.Not.Null);
        UInt16 rsaLen = (UInt16)((env![2] << 8) | env[3]);
        ...
        Assert.That(CryptoCore.Decrypt(env, cert), Is.Null);
        env[index] = 20;
        Assert.That(CryptoCore.Decrypt(env, cert), Is.Not.Null);
    }
}