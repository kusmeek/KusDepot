...
public class KusDepotCabExam
{
    ...
    public void Roundtrip_KeySet_Locked_Gating()
    {
        ...
        var back = parsed!.GetKeySet();
        Check.That(back).IsNotNull();
        Check.That(back!.GetAllKeys()).IsNull();
        var all = back.GetAllKeys(KusDepot.Exams.DataItems.DataItemSecurityContextExamHelpers.CreateContext(back,owner!));
        Check.That(all).IsNotNull();
    ...
    public async Task Roundtrip_Complex_Nested_Encrypted_Signed_Verified()
    {
        ...
        Assert.That(ks.AddKeys(originalKeys, KusDepot.Exams.DataItems.DataItemSecurityContextExamHelpers.CreateContext(ks,ksOwner!), true), Is.True);
        var ksOwnerContext = KusDepot.Exams.DataItems.DataItemSecurityContextExamHelpers.CreateContext(ks,ksOwner!);
        Assert.That(await ks.EncryptData(ksOwnerContext), Is.True);
        Assert.That(await ks.VerifyData("EncryptedData", ksOwnerContext), Is.True);
        ...
        var innerGIOwnerContext = KusDepot.Exams.DataItems.DataItemSecurityContextExamHelpers.CreateContext(innerGI, innerGIOwner!);
        var innerGISignerContext = KusDepot.Exams.DataItems.DataItemSecurityContextExamHelpers.CreateContext(innerGI, innerGISigner!);

        Assert.That(innerGI.SignData("Content", innerGISignerContext), Is.Not.Null);
        Assert.That(await innerGI.EncryptData(innerGIOwnerContext), Is.True);
        Assert.That(await innerGI.VerifyData("EncryptedData", innerGIOwnerContext), Is.True);
        ...
        var outerGISignerContext = KusDepot.Exams.DataItems.DataItemSecurityContextExamHelpers.CreateContext(outerGI, outerGISigner!);

        Assert.That(outerGI.SignData("Content", outerGISignerContext), Is.Not.Null);
        Assert.That(await outerGI.VerifyData("Content", outerGISignerContext), Is.True);
        ...
        Assert.That(await extractedOuterGI!.VerifyData("Content", KusDepot.Exams.DataItems.DataItemSecurityContextExamHelpers.CreateContext(extractedOuterGI,outerGISigner!)), Is.True);
        ...
        var extractedInnerGIOwnerContext = KusDepot.Exams.DataItems.DataItemSecurityContextExamHelpers.CreateContext(extractedInnerGI,innerGIOwner!);
        Assert.That(await extractedInnerGI.VerifyData("EncryptedData", extractedInnerGIOwnerContext), Is.True);
        Assert.That(await extractedInnerGI.DecryptData(extractedInnerGIOwnerContext), Is.True);
        Assert.That(await extractedInnerGI.VerifyData("Content", extractedInnerGIOwnerContext), Is.True);
        Assert.That(await extractedInnerGI.VerifyData("Content", KusDepot.Exams.DataItems.DataItemSecurityContextExamHelpers.CreateContext(extractedInnerGI,innerGISigner!)), Is.False);

        var extractedKS = extractedInnerGI.GetContent()!.ToList().OfType<KeySet>().Single();
        Assert.That(extractedKS, Is.Not.Null);

        Assert.That(await extractedKS.VerifyData("EncryptedData", KusDepot.Exams.DataItems.DataItemSecurityContextExamHelpers.CreateContext(extractedKS,ksOwner!)), Is.True);
        Assert.That(await extractedKS.DecryptData(KusDepot.Exams.DataItems.DataItemSecurityContextExamHelpers.CreateContext(extractedKS,ksOwner!)), Is.True);
