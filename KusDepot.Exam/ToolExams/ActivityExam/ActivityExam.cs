...
public class ActivityExam
{
    ...
    public void CreateActivity()
    {
        ...
        var act = Activity.CreateActivity(details, cmd, asynchronous: true);
        ...
        Check.That(act.FreeMode).IsFalse();
    }

    [Test]
    public async Task CreateActivity_UsesOwnedTimeoutCts_WhenNoUpstreamTokenCanCancel()
    {
        var details = new CommandDetails(
            arguments: new Dictionary<String, Object?>
            {
        ...
        var act = Activity.CreateActivity(details, new FreeModeCommand(), asynchronous: true);

        Check.That(act).IsNotNull();
        Check.That(act!.Cancel).IsNotNull();
        Check.That(act.Cancel!.Token.CanBeCanceled).IsTrue();
        Check.That(details.GetCancel().CanBeCanceled).IsTrue();

        await Task.Delay(150);

        Check.That(act.Cancel.IsCancellationRequested).IsTrue();
        Check.That(details.GetCancel().IsCancellationRequested).IsTrue();
    }

    [Test]
    public async Task CreateActivity_UsesLinkedCts_WhenUpstreamTokenCanCancel()
    {
    ...
    [Test]
    public void Records_SetRecordByTool_WhenNotFreeMode()
    {
        var act = new Activity();

        Check.That(act.FreeMode).IsFalse();
        Check.That(act.Record).IsNull();
        Check.That(act.SetRecordByTool(ActivityRecordCode.Success, "ok")).IsTrue();
        Check.That(act.Record).IsNotNull();

        Check.That(act.SetRecordByTool(ActivityRecordCode.Faulted, "no")).IsFalse();
    }

    [Test]
    public void Records_SetRecordByCommand_IgnoresFreeMode()
    {
        var act = new Activity() { FreeMode = true };

        Check.That(act.Record).IsNull();
        Check.That(act.SetRecordByCommand(ActivityRecordCode.Success, "cmd")).IsTrue();
        Check.That(act.Record).IsNotNull();

        Check.That(act.SetRecordByCommand(ActivityRecordCode.Faulted, "again")).IsFalse();
    }

    [Test]
    public void Records_SetRecordByTool_NoOp_WhenFreeModeTrue()
    {
        var act = new Activity() { FreeMode = true };

        Check.That(act.Record).IsNull();
        Check.That(act.SetRecordByTool(ActivityRecordCode.Success, "tool")).IsFalse();
        Check.That(act.Record).IsNull();
    }
