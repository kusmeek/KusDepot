...
public partial class ToolDataConverterExam
{
    ...
    private static IEnumerable<TestCaseData> GetAllKnownTypeWriteCases()
    {
        ...
        using(var requestcert = CreateCertificate(Guid.NewGuid(), "ManagementRequest"))
        {
            yield return WriteCase((Object)new ManagementRequest(new ManagerKey(requestcert!, Guid.NewGuid())), typeof(ManagementRequest), ToolValueMode.Parse, "KnownType_Write_ManagementRequest");
        }
    }
    ...
    private static IEnumerable<TestCaseData> GetReadableKnownTypeRoundtripCases()
    {
        ...
        yield return ReadCase(new ToolValue { Mode = ToolValueMode.Parse, Type = typeof(TextItem).FullName, Data = new TextItem("hello").ToString() }, typeof(TextItem), "KnownType_Read_TextItem");
        yield return ReadCase(new ToolValue { Mode = ToolValueMode.Parse, Type = typeof(CodeItem).FullName, Data = new CodeItem("code").ToString() }, typeof(CodeItem), "KnownType_Read_CodeItem");
        yield return ReadCase(new ToolValue { Mode = ToolValueMode.Parse, Type = typeof(BinaryItem).FullName, Data = new BinaryItem(new Byte[] { 1, 2, 3 }).ToString() }, typeof(BinaryItem), "KnownType_Read_BinaryItem");
        yield return ReadCase(new ToolValue { Mode = ToolValueMode.Parse, Type = typeof(MSBuildItem).FullName, Data = new MSBuildItem("build").ToString() }, typeof(MSBuildItem), "KnownType_Read_MSBuildItem");
        yield return ReadCase(new ToolValue { Mode = ToolValueMode.Parse, Type = typeof(GenericItem).FullName, Data = new GenericItem(new Object[] { "nested", 42 }).ToString() }, typeof(GenericItem), "KnownType_Read_GenericItem");
        yield return ReadCase(new ToolValue { Mode = ToolValueMode.Parse, Type = typeof(DataSetItem).FullName, Data = new DataSetItem(new DataItem[] { new TextItem("nested") }).ToString() }, typeof(DataSetItem), "KnownType_Read_DataSetItem");
        yield return ReadCase(new ToolValue { Mode = ToolValueMode.Parse, Type = typeof(MultiMediaItem).FullName, Data = new MultiMediaItem().ToString() }, typeof(MultiMediaItem), "KnownType_Read_MultiMediaItem");
        yield return ReadCase(new ToolValue { Mode = ToolValueMode.Parse, Type = typeof(GuidReferenceItem).FullName, Data = new GuidReferenceItem(Guid.NewGuid()).ToString() }, typeof(GuidReferenceItem), "KnownType_Read_GuidReferenceItem");
        yield return ReadCase(new ToolValue { Mode = ToolValueMode.Parse, Type = typeof(KeySet).FullName, Data = new KeySet(new SecurityKey[] { new ServiceKey(Bytes(10), Guid.NewGuid()) }).ToString() }, typeof(KeySet), "KnownType_Read_KeySet");
        ...
        using(var requestcert = CreateCertificate(Guid.NewGuid(), "ManagementRequest"))
        {
            yield return ReadCase(new ToolValue { Mode = ToolValueMode.Custom, Type = typeof(ManagementRequest).FullName, Data = JsonUtility.ToJsonString(new ManagementRequest(new ManagerKey(requestcert!, Guid.NewGuid())), typeof(ManagementRequest)) }, typeof(ManagementRequest), "KnownType_Read_ManagementRequest");
        }
    }
