...
public class ToolSecurityExam
{
    ...
    [Test]
    public void CreateExecutiveKey()
    {
        Tool _ = new();
        ...
        var ko = _.CreateExecutiveKey(KeyO!); Check.That(ko).IsInstanceOf<ExecutiveKey>();

        var km = _.CreateExecutiveKey(KeyM!); Check.That(km).IsInstanceOf<ExecutiveKey>();
    }
    ...
    [Test]
    public void Lock_AccessKey()
    {
        Tool tool = new();

        Check.That(tool.TakeOwnership(KeyO)).IsTrue();

        ExecutiveKey? key = tool.RequestAccess(new ManagementRequest(KeyO!)) as ExecutiveKey;

        Check.That(key).IsNotNull();
        Check.That(tool.GetLocked()).IsFalse();
        Check.That(tool.Lock(key)).IsTrue();
        Check.That(tool.GetLocked()).IsTrue();
    }
    ...
    public void RequestAccess()
    {
        ...
        Check.That(_0.RequestAccess(new ServiceRequest(_1,_1.GetID()!.Value.ToStringInvariant()!))).IsInstanceOf<ServiceKey>();

        Check.That(_0.RequestAccess(new ServiceRequest(_0,_0.GetID()!.Value.ToStringInvariant()!))).IsInstanceOf<ExecutiveKey>();

        Check.That(_0.RegisterManager(KeyM)).IsTrue(); Check.That(_0.Lock(KeyM)).IsTrue();
        ...
        Check.That(_0.RequestAccess(new ServiceRequest(_0,_0.GetID()!.Value.ToStringInvariant()!))).IsNull();

        Check.That(_0.RequestAccess(new ServiceRequest(_1,_1.GetID()!.Value.ToStringInvariant()!))).IsNull();
    ...
    public void RevokeAccess()
    {
        ...
        ServiceKey? k4 = _0.RequestAccess(new ServiceRequest(_1,_1.GetID()!.Value.ToStringInvariant()!)) as ServiceKey; Check.That(k4).IsNotNull();
    ...
    public void UnLock_AccessKey()
    {
        ...
        Check.That(tool.Lock(key)).IsTrue();
        Check.That(tool.GetLocked()).IsTrue();
        Check.That(tool.UnLock(key)).IsTrue();
        Check.That(tool.GetLocked()).IsFalse();
    }
    ...
    public async Task DestroySecrets()
    {
        ...
        var destroySecretsManagerMethod = typeof(Tool).GetMethod("DestroySecrets", BindingFlags.NonPublic | BindingFlags.Instance, null, new[] { typeof(ManagementKey) }, null);
        var destroySecretsAccessMethod = typeof(Tool).GetMethod("DestroySecrets", BindingFlags.NonPublic | BindingFlags.Instance, null, new[] { typeof(AccessKey) }, null);
        Check.That(destroySecretsManagerMethod).IsNotNull();
        Check.That(destroySecretsAccessMethod).IsNotNull();

        var resultFalse = (Boolean)destroySecretsManagerMethod!.Invoke(tool, new Object?[] { new ManagerKey(Array.Empty<Byte>()) })!;
        Check.That(resultFalse).IsFalse();
        Check.That(getSecrets(tool)).IsNotNull().And.Not.IsEmpty();

        var selfKey = typeof(Tool).GetField("SelfKey", BindingFlags.Instance | BindingFlags.NonPublic)!.GetValue(tool) as AccessKey;
        Check.That(selfKey).IsNotNull();

        var resultTrue = (Boolean)destroySecretsAccessMethod!.Invoke(tool, new Object?[] { selfKey })!;
