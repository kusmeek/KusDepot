...
public partial class ToolExam
{
    ...
    [Test]
    public async Task StopHostAsync_StopFaultService_FailsAndLeavesHostedService()
    {
        using Tool host = CreateHost();
        ...
        Check.That(await host.StopHostAsync()).IsFalse();
        Check.That(host.GetLifeCycleState()).IsEqualTo(LifeCycleState.Error);
    ...
    [Test]
    public async Task StartStopHostAsync_Concurrent_PlainHostedService_StartsAndStops()
    {
        using ToolHostingOptionsControl host = new(servicesstartconcurrently:true,servicesstopconcurrently:true);

        var service = new HostedServiceCounter();

        Check.That(await host.AddHostedService(service,start:false)).IsTrue();
        Check.That(service.StartCount).IsEqualTo(0);
        Check.That(service.StopCount).IsEqualTo(0);

        Check.That(await host.StartHostAsync()).IsTrue();
        Check.That(host.GetLifeCycleState()).IsEqualTo(LifeCycleState.Active);
        Check.That(service.StartCount).IsEqualTo(1);
        Check.That(service.StopCount).IsEqualTo(0);

        Check.That(await host.StopHostAsync()).IsTrue();
        Check.That(host.GetLifeCycleState()).IsEqualTo(LifeCycleState.InActive);
        Check.That(service.StartCount).IsEqualTo(1);
        Check.That(service.StopCount).IsEqualTo(1);
    }

    [Test]
    public async Task StartStopHostAsync_Sequential_PlainHostedService_StartsAndStops()
    {
        using ToolHostingOptionsControl host = new(servicesstartconcurrently:false,servicesstopconcurrently:false);
