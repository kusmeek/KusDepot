...
public sealed class HostedServiceCounter : IHostedService
{
    ...
    {
        this.StartCount++; return Task.CompletedTask;
    }

    public Task StopAsync(CancellationToken cancellationToken)
    {
        this.StopCount++; return Task.CompletedTask;
    }
}
...
{
    public ToolHostingOptionsControl(TimeSpan? shutdown = null , TimeSpan? startup = null , Boolean servicesstartconcurrently = true , Boolean servicesstopconcurrently = true) : base()
    {
        this.HostingOptions = new ToolHostOptions
        {
            ServicesStartConcurrently = servicesstartconcurrently,
            ServicesStopConcurrently = servicesstopconcurrently,
...
{
    public TestAccessManager(ITool? tool = null , Microsoft.Extensions.Logging.ILogger? logger = null) : base(tool,logger){}
    ...
    protected override AccessPolicyRequestContext CreateRequestPolicyContext(AccessRequest request , ITool tool)
    {
        ...
        AccessPolicyRequestContext CreateContext(Boolean requestvalidated , String requestedkeytype , Boolean externalrequest = false , Boolean hostmatched = false , Boolean servicematched = false)
        {
            return new()
            {
        ...
        switch(request)
        {
            ...
            {
                return base.CreateRequestPolicyContext(request,tool);
            }
            ...
            case HostRequest hostrequest:
            {
                ...
                return CreateContext(hostrequest.External || hostmatched,NormalizeRequestedKeyType(nameof(HostKey))!,externalrequest: hostrequest.External,hostmatched: hostmatched);
            }
            ...
            case ServiceRequest servicerequest:
            {
                ...
                return CreateContext(servicetoolvalid,NormalizeRequestedKeyType(servicematched ? nameof(ExecutiveKey) : nameof(ServiceKey))!,servicematched: servicematched) with
                {
                    Operations = servicerequest.Operations ?? ( servicematched ? ProtectedOperationSets.Executive : ProtectedOperationSets.Service )
                };
            }

            case StandardRequest:
            {
                return CreateContext(true,NormalizeRequestedKeyType(nameof(ClientKey))!);
            }
