...
{
    private static Byte[] Rand64()
    {
        Byte[] b = new Byte[64]; RandomNumberGenerator.Fill(b); return b;
    }

    [Test]
    public void Construct_And_ToString_HexLength()
    {
        var raw = Rand64(); var tok = new AccessKeyToken(raw);
        String hex = tok.ToString();
        Check.That(hex).IsNotNull();
        Check.That(hex.Length).IsEqualTo(128);
    }

    [Test]
    public void Equality_SameBytes()
    {
        var raw = Rand64(); var a = new AccessKeyToken(raw); var b = new AccessKeyToken(raw);
    ...
    {
        var a = new AccessKeyToken(Rand64()); var b = new AccessKeyToken(Rand64());
        Check.That(a == b).IsFalse();
        Check.That(a != b).IsTrue();
    }
    ...
    {
        var original = new AccessKeyToken(Rand64());
    ...
    public void MultipleRandomTokens_Unique()
    {
        ...
        {
            var t = new AccessKeyToken(Rand64());
