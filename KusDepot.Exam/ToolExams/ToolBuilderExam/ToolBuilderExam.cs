...
public class ToolBuilderExam
{
    ...
    public void Build()
    {
        Check.That(ToolBuilderFactory.CreateBuilder().Build().GetType().IsAssignableTo<ITool>()).IsTrue();
    }

    [Test]
    public void BuildOpen()
    {
        Check.That(ToolBuilderFactory.CreateBuilder().Build<Tool>().GetType().IsAssignableTo<ITool>()).IsTrue();
    }

    [Test]
    public async Task BuildAsync()
    {
        Check.That((await ToolBuilderFactory.CreateBuilder().BuildAsync()).GetType().IsAssignableTo<ITool>()).IsTrue();
    }

    [Test]
    public async Task BuildAsyncOpen()
    {
        Check.That((await ToolBuilderFactory.CreateBuilder().BuildAsync<Tool>()).GetType().IsAssignableTo<ITool>()).IsTrue();
    }
