...
public partial class ToolExam
{
    ...
    public async Task Activate()
    {
        ...
        Check.That(_.GetLifeCycleState()).Equals(LifeCycleState.Initialized);
    ...
    {
        Tool _0 = new Tool();
        Activity _1 = new Activity() { Task = Task.FromResult<Object?>(true) };
        Activity _2 = new Activity() { Task = Task.FromResult<Object?>(false) };
    ...
    [Test]
    public async Task AttachDetachCommand()
    {
        Tool _0 = new Tool();
        CommandTestE _1 = new CommandTestE();

        Check.That(_1.Enabled).IsFalse();

        Check.That(await _0.AttachCommand(_1,null,true)).IsTrue();
        Check.That(_1.Enabled).IsTrue();
        Check.That(_1.GetLocked()).IsTrue();

        Check.That(await _0.RegisterCommand("AttachDetach",_1)).IsTrue();
        Check.That(await _0.RegisterCommand("AttachDetach2",_1)).IsTrue();
        Check.That(await _0.UnRegisterCommand("AttachDetach",detach:true)).IsTrue();

        Check.That(_1.Enabled).IsTrue();
        Check.That(_1.GetLocked()).IsTrue();

        Check.That(await _0.UnRegisterCommand("AttachDetach2",detach:true)).IsTrue();

        Check.That(_1.GetLocked()).IsFalse();
        Check.That(_1.Enabled).IsFalse();
        Check.That(await _0.DetachCommand(_1)).IsFalse();
    }

    [Test]
    public async Task GetAttachedCommands()
    {
        Tool _0 = new Tool();
        CommandTestE _1 = new CommandTestE();
        CommandTestE _2 = new CommandTestE();

        Check.That(await _0.AttachCommand(_1,null,true)).IsTrue();
        Check.That(await _0.AttachCommand(_2,null,false)).IsTrue();

        Check.That(await _0.RegisterCommand("AttachedOnce",_1)).IsTrue();
        Check.That(await _0.RegisterCommand("AttachedTwice",_1)).IsTrue();

        var attached = _0.GetAttachedCommands();
        Check.That(attached).IsNotNull();
        Check.That(attached).Contains(_1,_2);
        Check.That(attached!.Count).IsEqualTo(2);
    }
    ...
    public void Calibrate()
    {
        ...
        KeyM = KeyMaker.Create().AutoID().WithCertificateKey(CreateCertificate(Guid.NewGuid(),"CN=Management")!).AsManagerKey();

        KeyO = KeyMaker.Create().AutoID().WithCertificateKey(CreateCertificate(Guid.NewGuid(),"CN=Owner")!).AsOwnerKey();
    ...
    [Test]
    public async Task DisableAllCommands()
    {
        var _ = ToolBuilderFactory.CreateBuilder()
            .RegisterCommand("OK",typeof(CommandTestE))
            .RegisterCommand("FAIL",typeof(CommandDisableFail))
            .Build();

        Check.That(await _.EnableAllCommands()).IsTrue();

        Check.That(await _.Activate()).IsTrue();

        Check.That(_.ExecuteCommand(new(){Handle = "OK"})).HasAValue();

        Check.That(await _.DisableAllCommands()).IsFalse();

        Check.That(_.ExecuteCommand(new(){Handle = "OK"})).HasNoValue();

        Check.That(_.ExecuteCommand(new(){Handle = "FAIL"})).HasAValue();

        Check.That(await _.DisableCommand("FAIL")).IsFalse();

        Check.That(_.ExecuteCommand(new(){Handle = "FAIL"})).HasAValue();
    }

    [Test]
    public async Task DisableCommand()
    {
        var _ = ToolBuilderFactory.CreateBuilder().RegisterCommand<CommandTestE>("Work").Build();

        Check.That(await _.Activate()).IsTrue();

        Check.That(_.ExecuteCommand(new(){Handle = "Work"})).HasNoValue();

        Check.That(await _.EnableCommand("Work")).IsTrue();

        Check.That(_.ExecuteCommand(new(){Handle = "Work"})).HasAValue();

        Check.That(await _.DisableCommand("Work")).IsTrue();
    ...
    public async Task EnableAllCommands()
    {
        ...
        Check.That(await _.EnableAllCommands()).IsFalse();

        Check.That(await _.Activate()).IsTrue();

        Check.That(_.ExecuteCommand(new(){Handle = "OK"})).HasAValue();

        Check.That(await _.EnableCommand("FAIL")).IsFalse();

        Check.That(_.ExecuteCommand(new(){Handle = "FAIL"})).HasNoValue();
    }

    [Test]
    public async Task EnableCommand()
    {
        var _ = ToolBuilderFactory.CreateBuilder().RegisterCommand<CommandTestE>("Work").Build();
    ...
    [Test]
    public async Task GetCommandDescriptor()
    {
        Tool _0 = new Tool();
        CommandTest _1 = new CommandTest();
        String _2 = "GetCommandDescriptorExam";
        String _3 = "GetCommandDescriptorExam2";

        Check.That(_0.GetCommandDescriptor(_2)).IsNull();
        Check.That(_0.GetCommandDescriptors()).IsNull();
        Check.That(await _0.RegisterCommand(_2,_1)).IsTrue();
        Check.That(await _0.RegisterCommand(_3,_1)).IsTrue();
        Check.That(_0.GetCommandDescriptor(_2)).IsNull();
        Check.That(_0.GetCommandDescriptors()).IsNull();

        Check.That(_0.UnMaskCommandTypes()).IsTrue();

        var d = _0.GetCommandDescriptor(_2);

        Check.That(d).IsNotNull();
        Check.That(d!.Type).Equals(_1.GetType().FullName!);
        Check.That(d.RegisteredHandles).Contains(_2,_3);
    }

    [Test]
    public async Task GetCommandDescriptors()
    {
        Tool _0 = new Tool();
        CommandTest _1 = new CommandTest();
        CommandTestR _2 = new CommandTestR();
        String _3 = "GetCommandDescriptorsExam";
        String _4 = "GetCommandDescriptorsExam2";
        String _5 = "GetCommandDescriptorsExam3";

        Check.That(await _0.RegisterCommand(_3,_1)).IsTrue();
        Check.That(await _0.RegisterCommand(_4,_1)).IsTrue();
        Check.That(await _0.RegisterCommand(_5,_2)).IsTrue();

        Check.That(_0.GetCommandDescriptors()).IsNull();

        Check.That(_0.UnMaskCommandTypes()).IsTrue();

        var d = _0.GetCommandDescriptors();

        Check.That(d).IsNotNull();
        Check.That(d!.Count).Equals(2);

        var d1 = d.First(_ => _.Type == _1.GetType().FullName!);
        var d2 = d.First(_ => _.Type == _2.GetType().FullName!);

        Check.That(d1.RegisteredHandles).Contains(_3,_4);
        Check.That(d2.RegisteredHandles).Contains(_5);
    }
    ...
    public void GetDataDescriptors()
    {
        ...
        Check.That(_d1.Count).Equals(_d2?.Count);
    }
    ...
    public void GetDataItem()
    {
        ...
        AccessKey? k = _0.RequestAccess(new ServiceRequest(_3,_3.GetID()!.Value.ToStringInvariant()!)
        {
            Operations = [ProtectedOperation.GetDataItem, ProtectedOperation.GetDataItems]
        });
    ...
    public void GetDataItems()
    {
        ...
        AccessKey? k = _7.RequestAccess(new ServiceRequest(_13,_13.GetID()!.Value.ToStringInvariant()!)
        {
            Operations = [ProtectedOperation.GetDataItems]
        });

        Check.That(k).IsNotNull();

        Check.That(_7.RegisterManager(KeyM)).IsTrue();
    ...
    public void GetRemoveOutput()
    {
        ...
        AccessKey? ks = _0.RequestAccess(new ServiceRequest(_3,_3.GetID()!.Value.ToStringInvariant()!)
        {
            Operations = [ProtectedOperation.GetOutput, ProtectedOperation.GetRemoveOutput, ProtectedOperation.RemoveOutput]
        });
    ...
    public void GetToolDescriptor()
    {
        ...
        var d = _0.GetToolDescriptor();

        Check.That(d).IsNotNull();
        Check.That(d!.ID).IsEqualTo(_0.GetID());
        Check.That(d.Type).IsEqualTo(_0.GetType().FullName);
    }
    ...
    public async Task RegisterCommand()
    {
        ...
        var d0 = _0.GetCommandDescriptors();

        Check.That(d0).IsNotNull();
        Check.That(d0!.Count).IsEqualTo(2);

        var d1 = d0.First(_ => _.Type == _1.GetType().FullName!);
        var d2 = d0.First(_ => _.Type == _2.GetType().FullName!);

        Check.That(d1.RegisteredHandles).Contains(_4,_5,_6);
        Check.That(d2.RegisteredHandles).Contains(_7);
        ...
        Check.That(await _0.RegisterCommand(_8,_10,null,true)).IsTrue();
        Check.That(await _0.RegisterCommand(_9,_11,null,true)).IsTrue();

        Check.That(_10.GetLocked()).IsTrue(); Check.That(_11.GetLocked()).IsTrue();

        var d3 = _0.GetCommandDescriptors();

        Check.That(d3).IsNotNull();
        Check.That(d3!.Count).IsEqualTo(4);
    }
    ...
    public async Task UnRegisterCommand()
    {
        ...
        Check.That(d3).IsNotNull();
        Check.That(d3!.Count).IsEqualTo(2);

        var d4 = d3.First(_ => _.Type == _1.GetType().FullName!);
        var d5 = d3.First(_ => _.Type == _2.GetType().FullName!);

        Check.That(d4.RegisteredHandles).Contains(_4,_6);
        Check.That(d5.RegisteredHandles).Contains(_7);
    }

    [Test]
    public async Task UnRegisterCommandDetach()
    {
        Tool _0 = new Tool();
        CommandTestE _1 = new CommandTestE();

        Check.That(await _0.RegisterCommand("Attach1",_1,null,true)).IsTrue();
        Check.That(await _0.RegisterCommand("Attach2",_1)).IsTrue();

        Check.That(await _0.UnRegisterCommand("Attach1",detach:true)).IsTrue();
        Check.That(_1.GetLocked()).IsTrue();

        Check.That(await _0.UnRegisterCommand("Attach2",detach:true)).IsTrue();
        Check.That(_1.GetLocked()).IsFalse();
        Check.That(_1.Enabled).IsFalse();
    }

    [Test]
    public async Task GetCommandHandles()
    {
        Tool _0 = new Tool();
        CommandTest _1 = new CommandTest();
        CommandTestR _2 = new CommandTestR();

        Check.That(await _0.RegisterCommand("Handle-A",_1)).IsTrue();
        Check.That(await _0.RegisterCommand("Handle-B",_1)).IsTrue();
        Check.That(await _0.RegisterCommand("Handle-C",_2)).IsTrue();

        var h1 = _0.GetCommandHandles(_1);
        var h2 = _0.GetCommandHandles(_2);

        Check.That(h1).IsNotNull();
        Check.That(h1!.Count).IsEqualTo(2);
        Check.That(h1).Contains("Handle-A","Handle-B");

        Check.That(h2).IsNotNull();
        Check.That(h2!.Count).IsEqualTo(1);
        Check.That(h2).Contains("Handle-C");

        Check.That(_0.GetCommandHandles(new CommandTestE())).IsNull();
        Check.That(_0.GetCommandHandles(null)).IsNull();
    }
