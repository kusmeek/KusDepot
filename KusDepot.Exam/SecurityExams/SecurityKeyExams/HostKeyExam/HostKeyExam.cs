...
public class HostKeyExam
{
    ...
    public void Equality_And_HashCode()
    {
        ...
        var k1 = new HostKey(data, id);
        var k2 = new HostKey(data.ToArray(), id);
        Check.That(k1).IsEqualTo(k2);
        Check.That(k1.GetHashCode()).IsEqualTo(k2.GetHashCode());

        var k3 = new HostKey(Array.Empty<Byte>());
        var k4 = new HostKey(Array.Empty<Byte>());
    ...
    {
        var k = new HostKey(new Byte[]{5});
        var s = k.ToString();

        var ak = AccessKey.Parse(s);
        Check.That(ak).IsNotNull();
        Check.That(ak).IsInstanceOf<HostKey>();

        var sk = SecurityKey.Parse(s);
        Check.That(sk).IsNotNull();
        Check.That(sk).IsInstanceOf<HostKey>();
    }

    [Test]
    public void Serialize_Parse_TryParse_Roundtrip()
    {
        var k = new HostKey(new Byte[]{1,2,3});
        var s = k.ToString();

        var kd = HostKey.Parse(s);
        Check.That(kd).IsNotNull();
        Check.That(kd).IsEqualTo(k);

        var success = HostKey.TryParse(s, null, out var kdp);
