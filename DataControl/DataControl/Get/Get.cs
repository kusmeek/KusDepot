...
public sealed partial class DataControl
{
    ...
    private async Task<IResult> Get(String? id , IBlob bl , ICoreCache cc , IDataConfigs dc , HttpContext hc)
    {
        try
        {
            ...
            if(String.IsNullOrEmpty(t)) { Log.Error(GetUnAuthID,id); SetErr(_); ETW.Log.GetError(GetUnAuth,id); return Results.Unauthorized(); }

            if(id is null || Equals(id,String.Empty)) { Log.Error(BadArgID,id); SetErr(_); ETW.Log.GetError(BadArgID,id); return Results.BadRequest(); }

            StorageSilo? s = await dc.GetAuthorizedReadSilo(t,dt,ds); if(s is null) { Log.Error(GetUnAuth,id); SetErr(_); ETW.Log.GetError(GetUnAuth,id); return Results.Unauthorized(); }
        ...
        catch ( Exception _ ) { Log.Error(_,GetFailID,id); ETW.Log.GetError(GetFail,id); return Results.Problem(id); }
