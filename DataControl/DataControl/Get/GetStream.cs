...
public sealed partial class DataControl
{
    ...
    private async Task<IResult> GetStream(String? id , IDataConfigs dc , HttpContext hc)
    {
        try
        {
            ...
            if(String.IsNullOrEmpty(id)) { Log.Error(BadArgID,id); SetErr(_); ETW.Log.GetStreamError(BadArgID,id); return Results.BadRequest(); }

            if(String.IsNullOrEmpty(t)) { Log.Error(GetUnAuthID,id); SetErr(_); ETW.Log.GetStreamError(GetUnAuth,id); return Results.Unauthorized(); }

            StorageSilo? s = await dc.GetAuthorizedReadSilo(t,dt,ds); if(s is null) { Log.Error(GetUnAuth,id); SetErr(_); ETW.Log.GetStreamError(GetUnAuth,id); return Results.Unauthorized(); }
            ...
            using MemoryStream m = new(); await bl.DownloadToAsync(m,hc.RequestAborted); String? it = m.ToArray().ToBase64FromByteArray(); if(String.IsNullOrEmpty(it)) { Log.Error(GetFailID,id); SetErr(_); ETW.Log.GetStreamError(GetFail,id); return Results.Problem(id); }
            ...
            if(File.Exists(p) is false) { Log.Error(GetFailID,id); SetErr(_); ETW.Log.GetStreamError(GetFail,id); return Results.Problem(id); }
        ...
        catch ( Exception _ ) { if(String.IsNullOrEmpty(id) is false && File.Exists(Path.Combine(DiskStorePath,id))) { File.Delete(Path.Combine(DiskStorePath,id)); } Log.Error(_,GetFailID,id); ETW.Log.GetError(GetFail,id); return Results.Problem(id); }
    }
}