...
public sealed partial class DataControl
{
    ...
    private async Task<IResult> StoreStream(DataControlUpload? dcu , Stream str , ICatalogDBFactory cdb , IDataConfigs dc , HttpContext hc)
    {
        try
        {
            ...
            {
                Log.Error(BadArg); SetErr(_); ETW.Log.StoreStreamError(BadArg,id); return Results.UnprocessableEntity($"{BadArg} {id}");
            }

            if(String.IsNullOrEmpty(t)) { Log.Error(StoreUnAuthID,id); SetErr(_); ETW.Log.StoreStreamError(StoreUnAuth,id); return Results.Unauthorized(); }

            StorageSilo? s = await dc.GetAuthorizedWriteSilo(t,dt,ds); String p = Path.Combine(DiskStorePath,id); FileStream f;

            if(s is null) { Log.Error(StoreUnAuthID,id); SetErr(_); ETW.Log.StoreStreamError(StoreUnAuth,id); return Results.Unauthorized(); } ICatalogDB c = cdb.Create(s.CatalogName);
            ...
            {
                Log.Error(StoreFailConflictID,id); SetErr(_); ETW.Log.StoreStreamError(StoreFailConflict,id); return Results.Conflict(id);
            }
            ...
            if(od.LiveStream is not true)
            {
                ...
                {
                    await f.DisposeAsync(); File.Delete(p); Log.Error(VerificationFailID,id); SetErr(_); ETW.Log.StoreStreamError(VerificationFail,id); return Results.UnprocessableEntity($"{VerificationFail} {id}");
                }
            }
            else
            {
        ...
        catch ( Exception _ ) { Log.Error(_,StoreFailDescriptor,dcu?.Descriptor); ETW.Log.StoreStreamError(_.Message,dcu?.Descriptor.ID?.ToString()); return Results.Problem(dcu?.Descriptor.ID.ToString()); }
    }
}