...
public sealed partial class DataControl
{
    ...
    private async Task<IResult> StoreStream(DataControlUpload? dcu , Stream str , ICatalogDBFactory cdb , IDataConfigs dc , HttpContext hc)
    {
        try
        {
            ...
            {
                if(await dcu.VerifyAsync(hc.RequestAborted) is false || String.Equals(dcu?.StreamSHA512,(await SHA512.HashDataAsync(f,hc.RequestAborted)).ToBase64FromByteArray(),Ordinal) is false)
                {
