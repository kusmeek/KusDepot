...
public sealed partial class DataControl
{
    ...
    private async Task<IResult> Store(DataControlUpload? dcu , ICatalogDBFactory cdb , IBlob bl , ICoreCache cc , IDataConfigs dc , HttpContext hc)
    {
        try
        {
            ...
            if(String.IsNullOrEmpty(t)) { Log.Error(StoreUnAuthID,id); SetErr(_); ETW.Log.StoreError(StoreUnAuth,id); return Results.Unauthorized(); }
            ...
            if(dcu.Verify() is false)
            {
                ...
                return Results.UnprocessableEntity($"{VerificationFail} {dcu?.Descriptor.ID.ToString()}");
            }

            StorageSilo? s = await dc.GetAuthorizedWriteSilo(t,dt,ds); if(s is null) { Log.Error(StoreUnAuthID,id); SetErr(_); ETW.Log.StoreError(StoreUnAuth,id); return Results.Unauthorized(); }

            ICatalogDB c = cdb.Create(s.CatalogName);

            if(await c.Exists(od,dt,ds) is not false)                          { Log.Error(StoreFailConflictID,id); SetErr(_); ETW.Log.StoreError(StoreFailConflict,id); return Results.Conflict(id); }

            if(await cc.Exists(id,dt,ds) is not false)                         { Log.Error(StoreFailConflictID,id); SetErr(_); ETW.Log.StoreError(StoreFailConflict,id); return Results.Conflict(id); }

            if(await bl.Exists(s.ConnectionString,id,null,dt,ds) is not false) { Log.Error(StoreFailConflictID,id); SetErr(_); ETW.Log.StoreError(StoreFailConflict,id); return Results.Conflict(id); }
            ...
            Log.Error(StoreBlobFailID,id); SetErr(_); ETW.Log.StoreError(StoreBlobFail,id); return Results.Problem(id);
        }
        catch ( Exception _ ) { Log.Error(_,StoreFailDescriptor,dcu?.Descriptor); ETW.Log.StoreError(_.Message,dcu?.Descriptor.ID?.ToString()); return Results.Problem(dcu?.Descriptor.ID.ToString()); }
