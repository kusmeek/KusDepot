...
public sealed partial class DataControl
{
    private void MapDelete(WebApplication application)
    {
        application.MapDelete("Delete/{id}",
                   ...
                   HttpContext hc) => {return Delete(id,bl,cdb,cm,dc,hc);})
                   .WithName("Delete").RequireAuthorization(X509Policy);
    }

    private async Task<IResult> Delete(String? id , IBlob bl , ICatalogDBFactory cdb , ICacheManager cm , IDataConfigs dc , HttpContext hc)
    {
        try
        {
