...
public sealed partial class DataControl
{
    ...
    private async Task<IResult> Delete(String? id , ICatalogDBFactory cdb , IBlob bl , ICoreCache cc , IDataConfigs dc , HttpContext hc)
    {
        try
        {
            ...
            if(String.IsNullOrEmpty(t)) { Log.Error(DeleteUnAuthID,id); SetErr(_); ETW.Log.DeleteError(DeleteUnAuth,id); return Results.Unauthorized(); }

            if(id is null || Equals(id,String.Empty)) { Log.Error(BadArgID,id); SetErr(_); ETW.Log.DeleteError(BadArg,id); return Results.BadRequest(); }

            StorageSilo? s = await dc.GetAuthorizedWriteSilo(t,dt,ds); if(s is null) { Log.Error(DeleteUnAuthID,id); SetErr(_); ETW.Log.DeleteError(DeleteUnAuth,id); return Results.Unauthorized(); }
            ...
            if(await bl.Exists(s.ConnectionString,id,null,dt,ds) is true)
            {
                ...
                Log.Error(DeleteBlobFailID,id); SetErr(_); ETW.Log.DeleteError(DeleteBlobFail,id); return Results.Problem(id);
            }
            Log.Error(DeleteNotFoundID,id); SetErr(_); ETW.Log.DeleteError(DeleteNotFound,id); return Results.NotFound(id);
        }
        catch ( Exception _ ) { Log.Error(_,DeleteFailID,id); ETW.Log.DeleteError(DeleteFail,id); return Results.Problem(id); }
