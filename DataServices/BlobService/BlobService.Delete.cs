        ...
        try
        {
            ...
            if(String.Equals(this.blobclient.Name,this.id,StringComparison.Ordinal))
            {
                if(await this.blobclient.DeleteIfExistsAsync(DeleteSnapshotsOption.IncludeSnapshots,cancellationToken:cancel))
                {
                    ...
                    BlobClient st = this.containerclient.GetBlobClient(BlobStrings.StreamBlobName); if(await st.ExistsAsync(cancel)) { await st.DeleteIfExistsAsync(cancellationToken:cancel); }

                    BlobClient it = this.containerclient.GetBlobClient(BlobStrings.IntegrityBlobName); if(await it.ExistsAsync(cancel)) { await it.DeleteIfExistsAsync(cancellationToken:cancel); }
