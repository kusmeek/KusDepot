...
public class ServiceManager : Tool , IManager
{
        ...
        try
        {
            Object? svcid = await GetRemoveOutputAsync(
                await ExecuteCommandAsync(
                    CommandDetails.Create("CheckServiceHealth").SetArgument("ManagerConfig",ManagerConfig).SetLogger(Logger),cancel,SelfKey)!,
                    cancel,key:SelfKey);
    ...
    protected virtual async Task<Boolean> TerminateService(CancellationToken? cancel = null)
    {
        try
        {
            ...
            return (Boolean?)await GetRemoveOutputAsync(
                await ExecuteCommandAsync(CommandDetails.Create("TerminateService").SetArgument("ManagerConfig",ManagerConfig).SetLogger(Logger).SetArgument("ProcessID",ServicePID),cancel,SelfKey)!,
    ...
    protected virtual async Task<Boolean> RestartService(CancellationToken? cancel = null)
    {
        ...
        {
            return (Boolean?)await GetRemoveOutputAsync(
                await ExecuteCommandAsync(CommandDetails.Create("RestartService").SetArgument("ManagerConfig",ManagerConfig).SetLogger(Logger).SetFreeMode(true),key:SelfKey)!,
                cancel,key:SelfKey) ?? false;
        }
    ...
    public override async Task StartingAsync(CancellationToken? cancel = null , AccessKey? key = null)
    {
        ...
        try
        {
            ...
            Logger?.Information(ManagerStarting,this.GetType().Name,ManagerConfig!.ToString()); await base.StartingAsync(cancel,key).ConfigureAwait(false);

            await Scheduler!.ScheduleJob(

                JobBuilder.Create<ManageJob>().UsingJobData("Manager",GetType().Name).Build(),

                TriggerBuilder.Create().WithSimpleSchedule(s => s.RepeatForever().WithInterval(TimeSpan.FromSeconds(ManagerConfig!.Interval))).Build()).ConfigureAwait(false);
        }
        catch ( Exception _ ) { Logger?.Error(_,StartingFail,this.GetType().Name); if(NoExceptions||MyNoExceptions) { return; } throw; }
    ...
    public override async Task StartedAsync(CancellationToken? cancel = null , AccessKey? key = null)
    {
        ...
        try
        {
            ...
            await base.StartedAsync(cancel,key).ConfigureAwait(false); Logger?.Information(ManagerStarted,this.GetType().Name,ManagerConfig!.ToString());
        }
        catch ( Exception _ ) { Logger?.Error(_,StartedFail,this.GetType().Name); if(NoExceptions||MyNoExceptions) { return; } throw; }
