...
public sealed class DaprSupervisor : Tool , IDaprSupervisor
{
    ...
    public override async Task StartingAsync(CancellationToken? cancel = null , AccessKey? key = null)
    {
        ...
        try
        {
            ...
            Managers.TryAdd(nameof(SidecarManager),(this.GetHostedServices(this.SelfKey)?.ToList().Find(_ => Equals(_.GetType(),typeof(SidecarManager))) as SidecarManager)!);

            Managers.TryAdd(nameof(PlacementManager),(this.GetHostedServices(this.SelfKey)?.ToList().Find(_ => Equals(_.GetType(),typeof(PlacementManager))) as PlacementManager)!);

            Managers.TryAdd(nameof(SchedulerManager),(this.GetHostedServices(this.SelfKey)?.ToList().Find(_ => Equals(_.GetType(),typeof(SchedulerManager))) as SchedulerManager)!);
