...
internal sealed class TerminateSidecar : Command
{
    public TerminateSidecar() { ExecutionMode.AllowBoth(); }
    ...
    private async Task<Object?> WorkAsync(Activity a)
    {
        try
        {
            AddActivity(a); var cfg = a.Details?.GetArgument<SidecarConfig>("SidecarConfig"); if(cfg is null) { SetFailed(a); AddOutput(a.ID); return false; }
