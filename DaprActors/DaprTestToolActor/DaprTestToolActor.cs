...
public partial class DaprTestToolActor : Actor , IDaprToolActor
{
    protected ITool? Tool;

    protected HostKey? HostKey;

    public DaprTestToolActor(ActorHost host) : base(host)
    {
        try
        {
            SetupLogging();
            ...
            Tool = ToolBuilderFactory.CreateBuilder()
                ...
                {
                    b.RegisterInstance(Log.Logger!).As<ILogger>();

                    b.RegisterInstance(host).AsImplementedInterfaces().AsSelf();
                })
        ...
        catch ( Exception _ ) { Log.Fatal(_,DaprToolActorFail,host.Id.ToString()); throw; }
    }
