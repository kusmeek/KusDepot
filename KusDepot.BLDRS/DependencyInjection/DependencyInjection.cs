...
public static class DependencyInjection
{
    ...
    /**<include file='DependencyInjection.xml' path='DependencyInjection/class[@name="DependencyInjection"]/method[@name="AddKeyedSingletonWithInterfaces_T"]/*'/>*/
    public static IServiceCollection AddKeyedSingletonWithInterfaces<T>(this IServiceCollection services , Object? key) where T : class
    {
        if(key is null) { return services; }

        services.AddKeyedSingleton<T,T>(key);

        foreach(var _ in typeof(T).GetInterfaces())
        {
            services.AddKeyedSingleton(_,key,(provider,_) => provider.GetRequiredKeyedService<T>(key));
        }

        return services;
    }

    /**<include file='DependencyInjection.xml' path='DependencyInjection/class[@name="DependencyInjection"]/method[@name="AddKeyedSingletonWithInterfaces_T_instance"]/*'/>*/
    public static IServiceCollection AddKeyedSingletonWithInterfaces<T>(this IServiceCollection services , Object? key , T instance) where T : class
    {
        if(key is null || instance is null) { return services; }

        services.AddKeyedSingleton<T>(key,instance);

        foreach(var _ in typeof(T).GetInterfaces())
        {
            services.AddKeyedSingleton(_,key,(provider,_) => instance);
        }
