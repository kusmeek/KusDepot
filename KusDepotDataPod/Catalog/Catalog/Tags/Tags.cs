...
public sealed partial class Catalog
{
    ...
    private async Task<IResult> SearchTags(TagQuery? search , IGrainFactory gf , HttpContext hc)
    {
        try
        {
            ...
            if(String.IsNullOrEmpty(tk)) { Log.Error(STGUnAuth); SetErr(_); return Unauthorized(); }

            if(search is null || search.Tags is null || Equals(search.Tags.Length,0)) { Log.Error(BadArg); SetErr(_); return BadRequest(BadArg); }

            var dc = gf.GetGrain<IDataConfigs>(Guid.NewGuid().ToStringInvariant()!);

            StorageSilo? s = await dc.GetAuthorizedReadSilo(tk,dt,ds,hc.RequestAborted); if(s is null) { Log.Error(STGUnAuth); SetErr(_); return Unauthorized(); }

            var c = gf.GetGrain<ICatalogDB>(s.CatalogName);

            var f = await c.SearchTags(search,dt,ds,hc.RequestAborted);

            if(Equals(f.IDs.Count,0)) { SetOk(_); return Results.NotFound(new TagResponse()); }

            SetOk(_); return Results.Ok(f);
        }
        catch ( Exception _ ) { Log.Error(_,STGFail); return InternalError(); }
    }
}