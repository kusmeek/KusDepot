...
{
    internal void ConfigureTLS(WebApplicationBuilder builder)
    {
    ...
    private const String ServerCertificatePasswordEnvironmentVariable = "KUSDEPOT_DATAPOD_CATALOG_CERTIFICATE_PASSWORD";

    private static X509Certificate2 GetLinuxServerCertificate()
    {
        if(File.Exists(CatalogServerCertificateFilePath) is false)
        {
            throw new FileNotFoundException("TLS server certificate file was not found.",CatalogServerCertificateFilePath);
        }

        return X509CertificateLoader.LoadPkcs12FromFile(CatalogServerCertificateFilePath,
