...
{
    public async Task<ElementResponse> SearchElements(ElementQuery? search , String? traceid = null , String? spanid = null , CancellationToken cancel = default)
    {
        ...
        try
        {
            ...
            IQueryable<KusDepot.CatalogDb.Element> q = ctx.Elements.AsQueryable();
            ...
            {
                q = q.Where(e => e.ObjectFilePath != null && EF.Functions.ILike(e.ObjectFilePath,$"%{search!.ObjectFILE!}%"));
            }
            ...
            {
                q = q.Where(e => e.DataType != null && EF.Functions.ILike(e.DataType,$"%{search!.DataType!}%"));
            }
            ...
            var results = await q.ToListAsync(cancel);

            if(results.Count == 0) { SetOk(da); return new ElementResponse(); }

            var ids = results.Select(r => r.ID!.Value).ToList();
            ...
            var output = results.Select(r => new Element
            {
            ...
            return new ElementResponse { Elements = output };
        }
        catch ( Exception _ )
        {
            Logger.Error(_,SELFail); SetErr(da);
