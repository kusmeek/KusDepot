...
{
    public async Task<ElementResponse> SearchElements(ElementQuery? search , String? traceid = null , String? spanid = null)
    {
        DiagnosticActivity? da = null;

        try
        {
            ETW.Log.SELStart(); da = StartDiagnostic(traceid,spanid);
            ...
            if(results.Count == 0) { SetOk(da); ETW.Log.SELSuccess(0); return new ElementResponse(); }
            ...
            SetOk(da); ETW.Log.SELSuccess(output.Length);

            return new ElementResponse { Elements = output };
        }
        catch ( Exception _ )
        {
            Logger.Error(_,SELFail); SetErr(da); ETW.Log.SELError(_.Message);
