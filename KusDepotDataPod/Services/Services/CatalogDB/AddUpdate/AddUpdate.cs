...
{
    public async Task<Boolean> AddUpdate(Descriptor? descriptor , String? traceid = null , String? spanid = null , CancellationToken cancel = default)
    {
        try
        {
            ...
            if(descriptor is null) { Logger.Error(BadArg); SetErr(__); return false; }

            cancel.ThrowIfCancellationRequested();

            await InitializeReadyDatabase(cancel); Boolean ok = await AddUpdateCore(descriptor,cancel);

            if(ok) { Logger.Information(AddUpdateSuccess,ct,descriptor); SetOk(__);return true; }

            Logger.Error(AddUpdateFailDescriptor,ct,descriptor); SetErr(__); return false;
        }
        catch ( Exception _ ) { Logger.Error(_,AddUpdateFailDescriptor,GetActorID(),descriptor); return false; }
    }
    ...
    private async Task<Boolean> AddUpdateCore(Descriptor d , CancellationToken cancel = default)
    {
        ...
        try
        {
            ...
            e.Name               = core.Name;
            e.ObjectFilePath     = core.ObjectFilePath;
