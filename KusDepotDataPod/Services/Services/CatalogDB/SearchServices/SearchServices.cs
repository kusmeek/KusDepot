...
{
    public async Task<ServiceResponse> SearchServices(ServiceQuery? search , String? traceid = null , String? spanid = null , CancellationToken cancel = default)
    {
        ...
        try
        {
            ...
            if(results.Count == 0) { SetOk(da); return new ServiceResponse(); }
            ...
            var tagsLookup = await ctx.Tags
                ...
                .GroupBy(tg => tg.ID)
                .ToDictionaryAsync(g => g.Key!.Value,g => g.Select(tg => tg.Value).ToArray(),cancel);
        ...
        {
            Logger.Error(_,SSVFail); SetErr(da);
