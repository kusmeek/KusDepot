...
public sealed partial class DataControl
{
    private void MapGet(WebApplication application)
    {
        application.MapGet("Get/{id}",
                   ...
                   HttpContext hc) => {return Get(id,bl,cm,gf,hc);})
                   .Produces<DataControlDownload>(StatusCodes.Status200OK)
                   .WithName("Get").RequireAuthorization(X509Policy);
    }

    private async Task<IResult> Get(String? id , IBlob bl , ICacheManager cm , IGrainFactory gf , HttpContext hc)
    {
        try
        {
            ETW.Log.GetStart(id);
            ...
            {
                Log.Information(GetSuccessCacheID,id); SetOk(_); ETW.Log.GetSuccess(id); return Results.Ok(MakeDataControlDownload(await cm.Get(id,dt,ds)));
            }
