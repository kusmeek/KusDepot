...
public sealed partial class DataControl
{
    private void MapGetStream(WebApplication application)
    {
        application.MapGet("GetStream/{id}",
                   ...
                   HttpContext hc) => { return GetStream(id,gf,hc); })
                   .WithName("GetStream").RequireAuthorization(X509Policy);
    }

    private async Task<IResult> GetStream(String? id , IGrainFactory gf , HttpContext hc)
    {
        try
        {
            ...
            String p = Path.Combine(DiskStorePath,id); if(!await bl.ExistsAsync(hc.RequestAborted) || !await bls.ExistsAsync(hc.RequestAborted) || !await blh.ExistsAsync(hc.RequestAborted)) { Log.Error(GetNotFoundID,id); SetErr(_); ETW.Log.GetStreamError(GetNotFound,id); return Results.NotFound(id); }
            ...
            if(File.Exists(p) is true)
            {
                ...
                if(String.Equals((await blh.DownloadContentAsync(hc.RequestAborted)).Value.Content.ToString(),(await SHA512.HashDataAsync(f,hc.RequestAborted)).ToBase64FromByteArray(),Ordinal) is not true)
                {
            ...
            hc.Response.Headers["DataControlDownload"] = (await MakeDataControlDownloadAsync(it,f,hc.RequestAborted))?.ToString();
