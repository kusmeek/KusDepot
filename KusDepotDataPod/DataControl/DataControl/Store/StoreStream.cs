...
public sealed partial class DataControl
{
    private void MapStoreStream(WebApplication application)
    {
        application.MapPost("StoreStream",
                   ...
                   HttpContext hc) => { return StoreStream(dcu,str,gf,hc); })
                   .WithName("StoreStream").RequireAuthorization(X509Policy);
    }

    private async Task<IResult> StoreStream(DataControlUpload? dcu , Stream str , IGrainFactory gf , HttpContext hc)
    {
        try
        {
            ...
            if(s is null) { Log.Error(StoreUnAuthID,id); SetErr(_); ETW.Log.StoreStreamError(StoreUnAuth,id); return Unauthorized(); } var c = gf.GetGrain<ICatalogDB>(s.CatalogName);
            ...
            if( await c.Exists(od,dt,ds)                        is not false ||
               (await bl.ExistsAsync(hc.RequestAborted)).Value  is not false ||
               (await blh.ExistsAsync(hc.RequestAborted)).Value is not false ||
               (await bls.ExistsAsync(hc.RequestAborted)).Value is not false  )
            {
            ...
            if(File.Exists(p))
            {
                ...
                if(od.LiveStream is true || String.Equals(dcu?.StreamSHA512,(await SHA512.HashDataAsync(f,hc.RequestAborted)).ToBase64FromByteArray(),Ordinal) is not true)
                {
            ...
            BlobContainerClient bc = new(s.ConnectionString,id); await bc.CreateIfNotExistsAsync(cancellationToken:hc.RequestAborted); f.Seek(0,SeekOrigin.Begin);
