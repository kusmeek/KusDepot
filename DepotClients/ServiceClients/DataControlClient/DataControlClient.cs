...
public sealed class DataControlClient : IDataControlClient , IDisposable
{
    ...
    ///<inheritdoc/>
    public async Task<RestResponse<DataControlDownload>> Get(Guid? id , String? token = null , CancellationToken cancel = default)
    {
        try
        {
            RestRequest _ = new RestRequest($"/Get/{id}",Method.Get); if(!String.IsNullOrEmpty(token)) { this.Auth.SetBearerToken(token); }

            return await this.Client.ExecuteAsync<DataControlDownload>(_,cancel).ConfigureAwait(false);
        }
        catch ( Exception _ ) { KusDepotLog.Error(_,GetFail); if(NoExceptions) { return null!; } throw; }
    }
    ...
    public async Task<DownloadInfo?> GetStream(Guid? id , String? filepath = null , String? itempath = null , String? token = null , CancellationToken cancel = default)
    {
        try
        {
            ...
            if(String.Equals(d.ObjectSHA512,(await SHA512.HashDataAsync(new MemoryStream(d.Object.ToByteArrayFromBase64()!),cancel)).ToBase64FromByteArray(),StringComparison.Ordinal) is false) { return null; }
    ...
    ///<inheritdoc/>
    public async Task<RestResponse<Guid>> Store(DataControlUpload dcu , String? token = null , CancellationToken cancel = default)
    {
        try
        {
            RestRequest _ = new RestRequest("/Store",Method.Post).AddJsonBody(dcu); if(!String.IsNullOrEmpty(token)) { this.Auth.SetBearerToken(token); }

            return await this.Client.ExecuteAsync<Guid>(_,cancel).ConfigureAwait(false);
        }
        catch ( Exception _ ) { KusDepotLog.Error(_,StoreFail); if(NoExceptions) { return null!; } throw; }
    }
