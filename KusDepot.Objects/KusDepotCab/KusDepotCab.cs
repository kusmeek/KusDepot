...
public record KusDepotCab : IParsable<KusDepotCab>
{
    ...
    public static KusDepotCab? Parse(String input) { try { return String.IsNullOrEmpty(input) ? null : JsonSerializer.Deserialize<KusDepotCab>(input); } catch ( Exception _ ) { Log.Error(_,ParseFail); return null; } }
}