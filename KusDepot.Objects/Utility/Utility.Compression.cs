...
public static partial class Utility
{
    ...
    public static Byte[]? Compress(this Byte[]? input)
    {
        try
        {
            if(input is null) { return null; } if(input.Length == 0) { return Array.Empty<Byte>(); }
    ...
    public static Boolean Compress(Stream? input , Stream? output , CancellationToken cancel = default)
    {
        try
        {
            ...
            using ZLibStream z = new(output,new ZLibCompressionOptions{ CompressionStrategy = ZLibCompressionStrategy.Default , CompressionLevel = 9 },leaveOpen:true);
    ...
    public static async Task<Boolean> CompressAsync(Stream? input , Stream? output , CancellationToken cancel = default)
    {
        try
        {
            ...
            await using ZLibStream z = new(output,new ZLibCompressionOptions{ CompressionStrategy = ZLibCompressionStrategy.Default , CompressionLevel = 9 },leaveOpen:true);
