...
public sealed partial class TypeProvider
{
    ...
    private Boolean TryResolve_NoSync()
    {
        try
        {
            ...
            if(TryGetEffectiveRequests_NoSync(out List<TypeRequest>? r) is false || r is null || r.Count == 0) { return false; }
            ...
            Boolean hasprobesources = (AssemblyPaths is not null && AssemblyPaths.Count != 0) ||
                (AssemblyDirectories is not null && AssemblyDirectories.Count != 0) ||
                (AssemblyImages is not null && AssemblyImages.Count != 0);
    ...
    private List<String> EnumerateCandidateAssemblyPaths_NoSync(TypeRequest request)
    {
        try
        {
            ...
            if(AssemblyPaths is not null && AssemblyPaths.Count != 0)
            {
            ...
            if(AssemblyDirectories is not null && AssemblyDirectories.Count != 0)
            {
    ...
    private Boolean TryResolveFromImagesInActiveContext_NoSync(TypeRequest request , out Type? type)
    {
        ...
        try
        {
            ...
            if(c is null)
            {
                if(AssemblyImages is null || AssemblyImages.Count == 0) { return false; }
    ...
    private Assembly? TryLoadImageByRequestedAssemblyInContext(AssemblyLoadContext context , AssemblyName requested)
    {
        try
        {
            if(AssemblyImages is null || AssemblyImages.Count == 0) { return null; }
    ...
    private static Boolean TypeRequestsEqual(TypeRequest left , TypeRequest right)
    {
        try
        {
            ...
            if(left.HasVersion != right.HasVersion) { return false; }

            if(left.Culture != right.Culture) { return false; }

            if(left.HasCulture != right.HasCulture) { return false; }

            if(String.Equals(left.PublicKeyToken,right.PublicKeyToken,Ordinal) is false) { return false; }

            if(left.HasPublicKeyToken != right.HasPublicKeyToken) { return false; }
