...
public abstract class Common : ICommon , IComparable<Common> , IEquatable<Common>
{
    ...
    [JsonPropertyName("Secrets")] [JsonInclude]
    [DataMember(Name = "Secrets" , EmitDefaultValue = true , IsRequired = true)] [Id(3)]
    protected internal List<Byte[]>? Secrets;
    ...
    public virtual Boolean CheckManager(ManagementKey? managementkey)
    {
        ...
        try
        {
            ...
            return this.CheckSecrets(managementkey?.Key,this.Secrets);
        }
    ...
    protected virtual Boolean CheckRemoveSecret(Byte[]? certificate , ICollection<Byte[]>? secrets)
    {
        try
        {
            if( secrets is null || secrets.Count == 0 ) { return false; }
            ...
            if(r.Count == 0) { return false; } foreach(var _ in r) { secrets.Remove(_); ZeroMemory(_); }
    ...
    protected virtual Byte[]? CreateSecret(Byte[]? certificate , Int32 depth = 0)
    {
        Boolean ss = false;
        try
        {
            ...
            if(this.Secrets is null || this.Secrets.Count == 0) { return s; }

            foreach(var _ in this.Secrets)
            {
                if(_.Length == s.Length && _.AsSpan().SequenceEqual(s)) { ZeroMemory(s); return CreateSecret(certificate,depth + 1); }
            }
    ...
    public virtual Boolean RegisterManager(ManagementKey? managementkey)
    {
        ...
        try
        {
            ...
            this.Secrets ??= new(); Byte[]? _ = CreateSecret(managementkey.Key); if(_ is null || _.Length == 0) { return false; }
    ...
    public virtual Boolean SetID(Guid? id)
    {
        ...
        try
        {
            ...
            if(id == Guid.Empty) { this.ID = null; return true; }
