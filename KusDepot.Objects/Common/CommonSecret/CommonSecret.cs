...
public sealed partial class CommonSecret
{
    ...
    public static CommonSecret? CreateV1(X509Certificate2? certificate , Guid? objectid)
    {
        ...
        Span<Byte> id = stackalloc Byte[GuidSize];
        ...
        try
        {
            ...
            if(objectid.Value.TryWriteBytes(id) is false) { return null; }
        ...
        finally { ZeroMemory(pt); ZeroMemory(salt); ZeroMemory(hash); ZeroMemory(cmb); ZeroMemory(aad); ZeroMemory(id); ZeroMemory(env); }
    }
    ...
    internal static Boolean TryValidateV1(ReadOnlySpan<Byte> secret , X509Certificate2? certificate , Guid? objectid)
    {
        ...
        try
        {
            if(objectid.Value.TryWriteBytes(id) is false) { return false; }
        ...
        finally { ZeroMemory(pt); ZeroMemory(aad); ZeroMemory(cmb); ZeroMemory(chash); ZeroMemory(id); }
    }
    ...
    public Byte[] GetBytes() { Byte[] _ = secret.ToArray(); ZeroMemory(secret); return _; }
}