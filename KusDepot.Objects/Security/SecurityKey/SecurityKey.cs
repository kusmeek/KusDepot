...
public abstract class SecurityKey : ICloneable , IEquatable<SecurityKey> , IParsable<SecurityKey>
{
    ...
    public Boolean ClearKey()
    {
        ...
        catch ( Exception _ ) { Log.Error(_,ClearKeyFail); if(NoExceptions) { return false; } throw; }
    }
    ...
    public TResult? Clone<TResult>() where TResult : SecurityKey
    {
        ...
        catch ( Exception _ ) { Log.Error(_,CloneFail); if(NoExceptions) { return null; } throw; }
    }
    ...
    public override String ToString()
    {
        ...
        catch ( Exception _ ) { Log.Error(_,ToStringFail); if(NoExceptions) { return String.Empty; } throw; }
    }
...
public record SecurityKeyWeb
{
    ...
    ///<inheritdoc/>
    public override String ToString() { try { return JsonSerializer.Serialize(this); } catch ( Exception _ ) { Log.Error(_,ToStringFail); return String.Empty; } }

    /**<include file='SecurityKey.xml' path='SecurityKey/record[@name="SecurityKeyWeb"]/method[@name="Parse"]/*'/>*/
    public static SecurityKeyWeb? Parse(String input) { try { return String.IsNullOrEmpty(input) ? null : JsonSerializer.Deserialize<SecurityKeyWeb>(input); } catch ( Exception _ ) { Log.Error(_,ParseFail); return null; } }
}