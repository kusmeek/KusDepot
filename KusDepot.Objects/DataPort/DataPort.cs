        ...
        try
        {
            ...
            switch(datatype)
            {
                case var t when String.Equals(t,DataTypes.GUID,Ordinal):
                {
                    ...
                    gri.SetContent(new Guid(bytes),(DataItemSecurityContext?)null);
                    item = gri;
                    break;
                }
                ...
                {
                    CodeItem ci = new();
                    if(contentStreamed is true) { if(await ci.SetContentStreamed(true,(DataItemSecurityContext?)null,cancel).ConfigureAwait(false) is false) { return null; } } else { ci.SetContent(bytes.TryGetStringFromByteArray(),(DataItemSecurityContext?)null); }
                    item = ci;
                    break;
                }
                ...
                {
                    TextItem ti = new();
                    if(contentStreamed is true) { if(await ti.SetContentStreamed(true,(DataItemSecurityContext?)null,cancel).ConfigureAwait(false) is false) { return null; } } else { ti.SetContent(bytes.TryGetStringFromByteArray(),(DataItemSecurityContext?)null); }
                    item = ti;
                    break;
                }
                ...
                case var t when DataValidation.IsValidMultiMediaDataType(t):
                {
                    ...
                    if(contentStreamed is true) { if(await mi.SetContentStreamed(true,(DataItemSecurityContext?)null,cancel).ConfigureAwait(false) is false) { return null; } } else { mi.SetContent(bytes,(DataItemSecurityContext?)null); }
                    item = mi;
                    break;
                }
                ...
                {
                    BinaryItem bi = new();
                    if(contentStreamed is true) { if(await bi.SetContentStreamed(true,(DataItemSecurityContext?)null,cancel).ConfigureAwait(false) is false) { return null; } } else { bi.SetContent(bytes,(DataItemSecurityContext?)null); }
                    item = bi;
                    break;
                }
