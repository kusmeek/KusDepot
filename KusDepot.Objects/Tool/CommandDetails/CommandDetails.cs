...
public record CommandDetailsWeb
{
    ...
    public static CommandDetailsWeb? Parse(String input) { try { return String.IsNullOrEmpty(input) ? null : JsonSerializer.Deserialize<CommandDetailsWeb>(input); } catch ( Exception _ ) { Log.Error(_,ParseFail); return null; } }

    ///<inheritdoc/>
    public override String ToString() { try { return JsonSerializer.Serialize(this,new JsonSerializerOptions(){ WriteIndented = true }); } catch ( Exception _ ) { Log.Error(_,ToStringFail); return String.Empty; } }
}
