...
public sealed class CommandDetails : IParsable<CommandDetails>
{
    ...
    public Boolean MakeReady(ITool tool)
    {
        try
        {
            ...
            if( this.id is null || this.id == Guid.Empty ) { this.id = Guid.NewGuid(); }
    ...
    public Object? GetArgument(String? name)
    {
        try
        {
            return String.IsNullOrEmpty(name) is false && this.Arguments?.TryGetValue(name,out Object? argument) is true ? argument : null;
        }
        catch { return null; }
    }
    ...
    public TResult? GetArgument<TResult>(String? name)
    {
        try
        {
            return String.IsNullOrEmpty(name) is false && this.Arguments?.TryGetValue(name,out Object? argument) is true ? (TResult?)argument : default;
        }
    ...
    public CommandDetails SetArgument(String? name = null , Object? argument = null , Boolean overwrite = false)
    {
        try
        {
            ...
            if(overwrite) { this.arguments[name] = argument; }

            else { _ = this.arguments.TryAdd(name,argument); }
        }
        catch {} return this;
    }
    ...
    {
        return OrleansUtility.ParseBase64<CommandDetails>(input);
    }
    ...
    {
        return OrleansUtility.TryParseBase64(input,out details);
    }
}