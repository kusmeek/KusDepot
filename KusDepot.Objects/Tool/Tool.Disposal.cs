...
public partial class Tool : Common , IHostedLifecycleService , ITool
{
    ...
    protected virtual void DisposeCore(Boolean disposing)
    {
        ...
        Logger?.Verbose(ToolDisposing,MyTypeName,MyID);
        ...
        try
        {
            if(disposing)
            {
                ...
                this.HostingKeys?.ToList().ForEach(_ => { _.Key.Dispose(_.Value); }); this.Container?.Dispose(); ((IDisposable?)this.Logger)?.Dispose(); this.DestroySecrets(this.SelfManagerKey);
    ...
    protected virtual async ValueTask DisposeAsyncCore(Boolean disposing)
    {
        ...
        try
        {
            if(disposing)
            {
                ...
                if(this.Container is not null) { await this.Container.DisposeAsync().ConfigureAwait(false); }

                if(this.Logger is IAsyncDisposable l) { await l.DisposeAsync().ConfigureAwait(false); } else { ((IDisposable?)this.Logger)?.Dispose(); }
