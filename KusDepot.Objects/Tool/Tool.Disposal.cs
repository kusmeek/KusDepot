...
public partial class Tool : Common , IHostedLifecycleService , ITool
{
    ...
    public virtual void Dispose(AccessKey? key = null)
    {
        ...
        try
        {
            ...
            if(this.LifeState.DisposeOK() is false) { return; }
    ...
    protected virtual void Dispose_NoSync(AccessKey? key = null)
    {
        try
        {
            if( this.Disposed || this.LifeState.DisposeOK() is false || this.AccessCheck(key) is false ) { return; }
    ...
    protected virtual void Dispose_NoSync()
    {
        try
        {
            if(this.Locked || this.Disposed || this.LifeState.DisposeOK() is false) { return; }
    ...
    {
        if(this.Disposed || this.LifeState.DisposeOK() is false) { return; }
        ...
        try
        {
            if(disposing)
            {
                ...
                this.DestroySecrets(this.SelfKey); this.ClearReferences();
            }
        }
        catch ( Exception _ ) { Logger?.Error(_,DisposeFail,MyTypeName,MyID); throw; }
    ...
    protected virtual async ValueTask DisposeAsync_NoSync(AccessKey? key = null)
    {
        try
        {
            if( this.Disposed || this.LifeState.DisposeOK() is false || await this.AccessCheckAsync(key).ConfigureAwait(false) is false ) { return; }
    ...
    protected virtual void ClearReferences()
    {
        ...
        this.HostedServiceNamesByInstance = null;
