...
public record AccessRequestWeb
{
    ...
    public static AccessRequestWeb? Parse(String input) { try { return String.IsNullOrEmpty(input) ? null : JsonSerializer.Deserialize<AccessRequestWeb>(input); } catch ( Exception _ ) { Log.Error(_,ParseFail); return null; } }
}