...
public sealed record class HostRequest : AccessRequest
{
    ...
    public HostRequest()
    {
        RequestedKeyType = nameof(HostKey); Operations = ProtectedOperationSets.Host;
    }

    /**<include file='HostRequest.xml' path='HostRequest/class[@name="HostRequest"]/constructor[@name="Constructor"]/*'/>*/
    public HostRequest(ITool? host , Boolean external = false)
    {
        Host = host; External = external; RequestedKeyType = nameof(HostKey); Operations = ProtectedOperationSets.Host;

        Subject = external ? Guid.NewGuid().ToString() : host?.GetID()?.ToStringInvariant();
    }
