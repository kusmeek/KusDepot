namespace KusDepot;

/**<include file='ToolOutput.xml' path='ToolOutput/class[@name="ToolOutput"]/main/*'/>*/
[KnownType("GetKnownTypes")]
[DataContract(Name = "ToolOutput" , Namespace = "KusDepot")]
public class ToolOutput : ToolData
{
    /**<include file='ToolOutput.xml' path='ToolOutput/class[@name="ToolOutput"]/property[@name="ID"]/*'/>*/
    [DataMember(Name = "ID" , EmitDefaultValue = true , IsRequired = true)] [NotNull]
    public Guid? ID { get; init; }

    /**<include file='ToolOutput.xml' path='ToolOutput/class[@name="ToolOutput"]/method[@name="GetKnownTypes"]/*'/>*/
    public static new IEnumerable<Type> GetKnownTypes() => GetAllKnownTypes();

    /**<include file='ToolOutput.xml' path='ToolOutput/class[@name="ToolOutput"]/method[@name="Parse"]/*'/>*/
    public static new ToolOutput? Parse(String input , IFormatProvider? format = null)
    {
        try
        {
            using XmlDictionaryReader _0 = XmlDictionaryReader.CreateBinaryReader(new MemoryStream(input.ToByteArrayFromBase64()),XmlDictionaryReaderQuotas.Max);

            DataContractSerializer _1 = new(typeof(ToolOutput),new DataContractSerializerSettings(){MaxItemsInObjectGraph = Int32.MaxValue});

            ToolOutput? _2 = _1.ReadObject(_0) as ToolOutput; if( _2 is not null ) { return _2; }

            throw new FormatException();
        }
        catch ( Exception _ ) { if(NoExceptions) { return null; } Log.Error(_,ParseFail); throw; }
    }

    /**<include file='ToolOutput.xml' path='ToolOutput/class[@name="ToolOutput"]/method[@name="ToString"]/*'/>*/
    public override String ToString()
    {
        try
        {
            MemoryStream _0 = new(); using XmlDictionaryWriter _1 = XmlDictionaryWriter.CreateBinaryWriter(_0);

            DataContractSerializer _2 = new(typeof(ToolOutput),new DataContractSerializerSettings(){MaxItemsInObjectGraph = Int32.MaxValue});

            _2.WriteObject(_1,this); _1.Flush(); _0.Seek(0,SeekOrigin.Begin);

            return _0.ToArray().ToBase64FromByteArray();
        }
        catch ( Exception _ ) { Log.Error(_,ToStringFail); if(NoExceptions) { return String.Empty; } throw; }
    }

    /**<include file='ToolOutput.xml' path='ToolOutput/class[@name="ToolOutput"]/method[@name="TryParse"]/*'/>*/
    public static Boolean TryParse([NotNullWhen(true)] String? input , IFormatProvider? format , [MaybeNullWhen(false)] out ToolOutput data)
    {
        data = null; if(input is null) { return false; }

        try
        {
            using XmlDictionaryReader _0 = XmlDictionaryReader.CreateBinaryReader(input.ToByteArrayFromBase64(),XmlDictionaryReaderQuotas.Max);

            DataContractSerializer _1 = new(typeof(ToolOutput),new DataContractSerializerSettings(){MaxItemsInObjectGraph = Int32.MaxValue});

            ToolOutput? _2 = _1.ReadObject(_0) as ToolOutput; if( _2 is not null ) { data = _2; return true; }

            return false;
        }
        catch ( Exception _ ) { if(NoExceptions) { return false; } Log.Error(_,TryParseFail); throw; }
    }
}