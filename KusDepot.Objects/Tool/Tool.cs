...
public partial class Tool : Common , IHostedLifecycleService , ITool
{
    ...
    /**<include file='Tool.xml' path='Tool/class[@name="Tool"]/field[@name="ContainerScope"]/*'/>*/
    protected Autofac.ILifetimeScope? ContainerScope;
    ...
    public Tool(IAccessManager? accessmanager = null , IEnumerable<DataItem>? data = null , Guid? id = null , IEnumerable<Meta<ICommand>>? commands = null,
           IConfiguration? configuration = null , IContainer? container = null , IEnumerable<Object>? inputs = null , ILogger? logger = null)
    {
        try
        {
            this.LifeState = null!; this.Sync = null!; this.Initialize(); this.SetID(id); this.SetContainer(container);

            this.ResolveLogger(logger); this.SetConfiguration(configuration); this.InitializeAccessManagement(accessmanager);

            this.ResolveAndUpdateInputs(inputs); this.RegisterCommands(commands); this.ResolveHostedServices(); this.AddDataItems(data);
        }
    ...
    public Tool() : this(null,null,null,null,null) {}
    ...
    protected static Boolean AddInstance(Tool? tool)
    {
        ...
        catch ( Exception _ ) { Log.Error(_,AddInstanceFail); if(NoExceptions) { return false; } throw; }
    ...
    protected override Boolean DestroySecrets(ManagementKey? managementkey)
    {
        ...
        catch ( Exception _ ) { Log.Error(_,DestroySecretsFail,MyTypeName,MyID); if(NoExceptions||MyNoExceptions) { return false; } throw; }
    ...
    /**<include file='Tool.xml' path='Tool/class[@name="Tool"]/method[@name="GetContainer"]/*'/>*/
    protected virtual Autofac.IContainer? GetContainer() { DC(); return this.Container; }

    ///<inheritdoc/>
    [AccessCheck(ProtectedOperation.GetContainerScope)]
    public virtual Autofac.ILifetimeScope? GetContainerScope(AccessKey? key = null)
    {
        DC(); try { if(this.AccessCheck(key) is false) { return null; } return this.ContainerScope; }

        catch ( Exception _ ) { Logger?.Error(_,GetContainerScopeFail,MyTypeName,MyID); if(NoExceptions||MyNoExceptions) { return null; } throw; }
    }
    ...
    public static ITool? GetInstance(Guid? id)
    {
        ...
        catch ( Exception _ ) { Log.Error(_,GetInstanceFail); if(NoExceptions) { return null; } throw; }
    ...
    public static IList<Guid>? GetInstanceIDs()
    {
        ...
        catch ( Exception _ ) { Log.Error(_,GetInstanceIDsFail); if(NoExceptions) { return null; } throw; }
    ...
    protected virtual Serilog.ILogger? GetLogger()
    {
    ...
    protected Boolean RegisterCommands(IEnumerable<Meta<ICommand>>? commands)
    {
        Boolean CS = false;
        DC();
        try
        {
            ...
            commands ??= this.GetContainerScope()?.ResolveOptional<IEnumerable<Meta<ICommand>>>() ?? Array.Empty<Meta<ICommand>>();

            var cmds = commands.ToList(); if(Equals(cmds.Count,0)) { return true; }
            ...
            foreach(Meta<ICommand> _ in cmds)
            {
                if(_.Metadata.TryGetValue("Handle", out var h) && h is String handle)
                {
                    if(this.Commands.ContainsKey(handle) || this.CommandTypes.ContainsKey(handle)) { throw new OperationFailedException(); }

                    if(_.Value is ICommand cmd)
                    {
                        CommandKey? k = this.CommandKeys!.GetValueOrDefault(cmd) as CommandKey; if(k is null) { k = this.AccessManager!.GenerateCommandKey(cmd,this.SelfAccessKey); if( k is null || this.CommandKeys!.TryAdd(cmd,k) is false) { throw new OperationFailedException(); } }
    ...
    protected static Boolean RemoveInstance(Tool? tool)
    {
        ...
        catch ( Exception _ ) { Log.Error(_,RemoveInstanceFail); if(NoExceptions) { return false; } throw; }
    ...
    protected virtual Boolean ResolveHostedServices()
    {
        ...
        try
        {
            ...
            this.HostingOptions = this.GetContainerScope()?.ResolveOptional<IOptions<ToolHostOptions>>()?.Value ?? new();

            this.HostedServices = this.GetContainerScope()?.ResolveOptional<IEnumerable<IHostedService>>()?.ToList() ?? new List<IHostedService>();
    ...
    /**<include file='Tool.xml' path='Tool/class[@name="Tool"]/method[@name="ResolveAndUpdateInputs"]/*'/>*/
    protected virtual Boolean ResolveAndUpdateInputs(IEnumerable<Object>? inputs)
    {
        DC();
        try
        {
            return this.UpdateInputs(inputs ?? this.GetContainerScope()?.ResolveOptional<IEnumerable<Object>>());
        }
        catch ( Exception _ ) { Logger?.Error(_,ResolveAndUpdateInputsFail,MyTypeName,MyID); if(NoExceptions||MyNoExceptions) { return false; } throw; }
    }

    /**<include file='Tool.xml' path='Tool/class[@name="Tool"]/method[@name="ResolveLogger"]/*'/>*/
    protected virtual Boolean ResolveLogger(Serilog.ILogger? logger = null)
    {
        DC();
        try
        {
            ...
            this.Logger = logger ?? this.GetContainerScope()?.ResolveOptional<Serilog.ILogger>();
    ...
    protected virtual Boolean SetConfiguration(IConfiguration? configuration)
    {
        DC();
        try
        {
            this.Configuration ??= ToolConfiguration.Deserialize(ToolConfiguration.Serialize(configuration  ??
                                   this.GetContainerScope()?.ResolveOptional<IConfiguration>())             ??
    ...
    /**<include file='Tool.xml' path='Tool/class[@name="Tool"]/method[@name="SetContainer"]/*'/>*/
    protected virtual Boolean SetContainer(Autofac.IContainer? container)
    {
        DC();
        try
        {
            ...
            if(!TryEnter(this.Sync.Container,SyncTime)) { throw SyncException; }

            this.Container = container; this.ContainerScope = this.Container.BeginLifetimeScope();

            return true;
        }
        catch ( Exception _ ) { Logger?.Error(_,SetContainerFail,MyTypeName,MyID); if(NoExceptions||MyNoExceptions) { return false; } throw; }
