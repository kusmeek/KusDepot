...
public sealed class KeySet : DataItem , IComparable<KeySet> , IEquatable<KeySet> , IParsable<KeySet>
{
        ...
        try
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    this.AllKeys[h] = key.Copy()!; MN();

                    if(!UpdateSecureHash(DataSecurityCNKey,this.GetIntegrityHash_NoSync()))                             { return false; }
            ...
            try
            {
                ...
                else
                {
                    ...
                    foreach(var k in nk)
                    {
                        ...
                        this.AllKeys[n!] = k.Copy()!; mod = true;
                    }
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        if(_.AllKeys is not null && _.AllKeys.Count > 0)
                        {
                            ...
                            foreach(var k in _.AllKeys) { this.AllKeys[k.Key] = k.Value.Copy()!; }

                            foreach(var k in _.AllKeys.Values) { k.ClearKey(); }
                        }
                        else { this.AllKeys = null; }

                        if(!await UpdateSecureHashAsync(DataSecurityCNKey,await this.GetIntegrityHash_NoSyncAsync(cancel),false,cancel).ConfigureAwait(false)) { return false; }
    ...
    public Boolean Equals(KeySet? other)
    {
        try
        {
            ...
            try
            {
                var a = this.AllKeys; var b = other.GetAllKeys();
    ...
    public static KeySet? FromFile(String path)
    {
        try
        {
            ...
            return DataContractUtility.FromFile<KeySet>(path,SerializationData.ForType(typeof(KeySet)));
        }
    ...
    protected internal override Byte[] GetIntegrityHash_NoSync()
    {
        try
        {
            var _ = (this.AllKeys?.Values ?? Enumerable.Empty<SecurityKey>()).OrderBy(v => v.ID)
                .SelectMany(v => SHA512.HashData(BitConverter.GetBytes(v.GetHashCode()))).ToArray();

            return SHA512.HashData(this.GetID()!.Value.ToByteArray().Concat(_).ToArray());
        }
        catch ( Exception _ ) { KusDepotLog.Error(_,GetHashCodeFail,MyTypeName,MyID); throw; }
    }
    ...
    protected internal override async Task<Byte[]> GetIntegrityHash_NoSyncAsync(CancellationToken cancel = default)
    {
        try
        {
            ...
            using var m = new MemoryStream(this.GetID()!.Value.ToByteArray().Concat(_).ToArray());
    ...
    public static new KeySet? Parse(String input , IFormatProvider? format = null)
    {
        try
        {
            return DataContractUtility.ParseBase64<KeySet>(input,SerializationData.ForType(typeof(KeySet)));
        }
    ...
    public static Boolean TryParse([NotNullWhen(true)] String? input , IFormatProvider? format , [MaybeNullWhen(false)] out KeySet key)
    {
        ...
        try
        {
            return DataContractUtility.TryParseBase64(input,SerializationData.ForType(typeof(KeySet)),out key);
        }
            ...
            try
            {
                switch(field)
                {
                    case DataSecurityCNKey:
                    {
                        ...
                        return await VerifyDataCore(field,managementkey,await this.GetIntegrityHash_NoSyncAsync(cancel).ConfigureAwait(false),null,cancel).ConfigureAwait(false);
                    }
