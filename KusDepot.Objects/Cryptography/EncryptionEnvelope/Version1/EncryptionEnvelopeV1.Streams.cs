...
{
    /**<include file='EncryptionEnvelopeV1.xml' path='EncryptionEnvelopeV1/class[@name="EncryptionEnvelopeV1"]/method[@name="DecryptStream"]/*'/>*/
    internal static Boolean DecryptStream(Stream? input , Stream? output , X509Certificate2? certificate , ReadOnlySpan<Byte> aad = default)
    {
        ...
        if(output is MemoryStream memoryOutput && TryGetRemainingMemory(input,out ReadOnlyMemory<Byte> decryptInput))
        {
            Byte[]? decrypted = DecryptArrayCore(decryptInput.Span,certificate,aad); if(decrypted is null) { return false; }

            memoryOutput.Write(decrypted,0,decrypted.Length);
        ...
        try
        {
            ...
            try
            {
                ...
                if(header.HasOriginalLength)
                {
                    ...
                    while(true)
                    {
                        if(ReadExact(input,lengths) is false) { break; }
    ...
    /**<include file='EncryptionEnvelopeV1.xml' path='EncryptionEnvelopeV1/class[@name="EncryptionEnvelopeV1"]/method[@name="DecryptStreamAsync"]/*'/>*/
    internal static async Task<Boolean> DecryptStreamAsync(Stream? input , Stream? output , X509Certificate2? certificate , ReadOnlyMemory<Byte> aad = default , CancellationToken cancel = default)
    {
        ...
        if(output is MemoryStream memoryOutput && TryGetRemainingMemory(input,out ReadOnlyMemory<Byte> decryptInput))
        {
            Byte[]? decrypted = DecryptArrayAsyncCore(decryptInput,certificate,aad,cancel); if(decrypted is null) { return false; }

            await memoryOutput.WriteAsync(decrypted.AsMemory(),cancel).ConfigureAwait(false);
        ...
        try
        {
                    ...
                    if(header.HasOriginalLength)
                    {
                        while(totalPlain < header.OriginalLength)
                        {
                            ...
                            try
                            {
                                ...
                                if(await ReadExactAsync(input,lengths.AsMemory(0,ChunkLengthsBlockSize),cancel).ConfigureAwait(false) is false) { break; }
            ...
            DecryptedChunk ProcessChunk(DecryptionChunk d)
            {
                ...
                try
                {
                    ...
                    gcm.Decrypt(nonce.AsSpan(0,NonceSize),d.Cipher.AsSpan(0,(Int32)d.CipherLength),d.Tag.AsSpan(0,TagSize),plain.AsSpan(0,(Int32)d.PlainLength),perChunkAad.AsSpan(0,PerChunkAadHashSize));
    ...
    /**<include file='EncryptionEnvelopeV1.xml' path='EncryptionEnvelopeV1/class[@name="EncryptionEnvelopeV1"]/method[@name="EncryptStream"]/*'/>*/
        ...
        Boolean includeaadhash = false , Boolean includeoriginallength = true , Int32 chunksizepower = DefaultChunkSizePower)
    {
        ...
        if(output is MemoryStream memoryOutput && TryGetRemainingMemory(input,out ReadOnlyMemory<Byte> encryptInput))
        {
            Byte[]? encrypted = EncryptArrayCore(encryptInput.Span,certificate,aad,includeaadhash,includeoriginallength,chunksizepower); if(encrypted is null) { return false; }

            memoryOutput.Write(encrypted,0,encrypted.Length);
    ...
    /**<include file='EncryptionEnvelopeV1.xml' path='EncryptionEnvelopeV1/class[@name="EncryptionEnvelopeV1"]/method[@name="EncryptStreamAsync"]/*'/>*/
        ...
        Boolean includeaadhash = false , Boolean includeoriginallength = true , Int32 chunksizepower = DefaultChunkSizePower , CancellationToken cancel = default)
    {
        ...
        if(output is MemoryStream memoryOutput && TryGetRemainingMemory(input,out ReadOnlyMemory<Byte> encryptInput))
        {
            Byte[]? encrypted = EncryptArrayAsyncCore(encryptInput,certificate,aad,includeaadhash,includeoriginallength,chunksizepower,cancel); if(encrypted is null) { return false; }

            await memoryOutput.WriteAsync(encrypted.AsMemory(),cancel).ConfigureAwait(false);
        ...
        try
        {
            ...
            Int32 chunkSize = 1 << chunksizepower; UInt64 totalWritten = 0; UInt32 chunkCount = 0;
            ...
            else { if(WriteFooter(output,chunkCount) is false) { return false; } }
            ...
            EncryptedChunk ProcessChunk(EncryptionChunk e)
            {
                ...
                try
                {
                    ...
                    gcm.Encrypt(nonce.AsSpan(0,NonceSize),e.Plain.AsSpan(0,e.ReadLength),cipher.AsSpan(0,e.ReadLength),tag.AsSpan(0,TagSize),perChunkAad.AsSpan(0,PerChunkAadHashSize));
    ...
    /**<include file='EncryptionEnvelopeV1.xml' path='EncryptionEnvelopeV1/class[@name="EncryptionEnvelopeV1"]/method[@name="EncryptUtf16"]/*'/>*/
    internal static Boolean EncryptUtf16(String? input , Stream? output , X509Certificate2? certificate , ReadOnlySpan<Byte> aad = default,
        Boolean includeaadhash = false , Boolean includeoriginallength = true , Int32 chunksizepower = DefaultChunkSizePower)
    {
        if(String.IsNullOrEmpty(input) || output is null || output.CanWrite is false || certificate is null) { return false; }
        ...
        try
        {
            UInt64 originalLength = checked((UInt64)input.Length * sizeof(Char));
            ...
            Int32 chunkSize = 1 << chunksizepower; Int32 charsPerChunk = chunkSize / sizeof(Char);
            Byte[] plain = ArrayPool<Byte>.Shared.Rent(chunkSize); Byte[] cipher = ArrayPool<Byte>.Shared.Rent(chunkSize);
            Span<Byte> tag = stackalloc Byte[TagSize]; Span<Byte> nonce = stackalloc Byte[NonceSize];
            Span<Byte> lengths = stackalloc Byte[ChunkLengthsBlockSize]; Span<Byte> perChunkAad = stackalloc Byte[PerChunkAadHashSize];
            UInt64 totalWritten = 0; UInt32 chunkIndex = 0;

            try
            {
                using var gcm = new AesGcm(aesKey,TagSize);

                for(Int32 charOffset = 0; charOffset < input.Length;)
                {
                    Int32 charCount = Math.Min(input.Length - charOffset,charsPerChunk);
                    Int32 byteCount = Encoding.Unicode.GetBytes(input.AsSpan(charOffset,charCount),plain.AsSpan(0,chunkSize));
                    charOffset += charCount;

                    if(includeoriginallength && (totalWritten + (UInt64)byteCount) > originalLength) { return false; }
                    if(DeriveNonce(aesKey,baseNonceSeed,chunkIndex,nonce) is false)                  { return false; }
                    if(BuildPerChunkAad((Byte)((includeoriginallength?Flag_HasOriginalLength:0) | (includeaadhash?Flag_HasAadHash:0)),chunkIndex,(UInt32)byteCount,includeoriginallength,originalLength,includeaadhash ? aadHash : ReadOnlySpan<Byte>.Empty,includeaadhash,perChunkAad) is false) { return false; }

                    gcm.Encrypt(nonce,plain.AsSpan(0,byteCount),cipher.AsSpan(0,byteCount),tag,perChunkAad);
                    WriteUInt32BigEndian(lengths[..IntSize],(UInt32)byteCount);
                    WriteUInt32BigEndian(lengths.Slice(IntSize,IntSize),(UInt32)byteCount);
                    output.Write(lengths);
                    output.Write(cipher,0,byteCount);
                    output.WriteByte(TagSize);
                    output.Write(tag);
                    totalWritten += (UInt64)byteCount;
                    chunkIndex++;
                }
        ...
        catch ( CryptographicException _ ) { KusDepotLog.Trace(_,EncryptUtf16Fail); return false; }

        catch ( Exception _ ) { KusDepotLog.Error(_,EncryptUtf16Fail); if(NoExceptions) { return false; } throw; }

        finally { ZeroMemory(aesKey); ZeroMemory(baseNonceSeed); ZeroMemory(aadHash); ZeroMemory(rsaWrappedKey); ZeroMemory(wrappedMaterial); }
    }

    /**<include file='EncryptionEnvelopeV1.xml' path='EncryptionEnvelopeV1/class[@name="EncryptionEnvelopeV1"]/method[@name="EncryptUtf16Async"]/*'/>*/
    internal static async Task<Boolean> EncryptUtf16Async(String? input , Stream? output , X509Certificate2? certificate , ReadOnlyMemory<Byte> aad = default,
        Boolean includeaadhash = false , Boolean includeoriginallength = true , Int32 chunksizepower = DefaultChunkSizePower , CancellationToken cancel = default)
    {
        if(String.IsNullOrEmpty(input) || output is null || output.CanWrite is false || certificate is null || cancel.IsCancellationRequested) { return false; }
        ...
        try
        {
            ...
            Int32 chunkSize = 1 << chunksizepower; UInt64 totalWritten = 0; UInt32 chunkIndex = 0;
            Int32 charsPerChunk = chunkSize / sizeof(Char); Byte[] tagLength = new[]{ (Byte)TagSize };
            Byte[] plain = ArrayPool<Byte>.Shared.Rent(chunkSize); Byte[] cipher = ArrayPool<Byte>.Shared.Rent(chunkSize);
            Byte[] tag = ArrayPool<Byte>.Shared.Rent(TagSize); Byte[] lengths = ArrayPool<Byte>.Shared.Rent(ChunkLengthsBlockSize);
            Byte[] nonce = ArrayPool<Byte>.Shared.Rent(NonceSize); Byte[] perChunkAad = ArrayPool<Byte>.Shared.Rent(PerChunkAadHashSize);
            ...
            try
            {
                ...
                for(Int32 charOffset = 0; charOffset < input.Length;)
                {
                    ...
                    gcm.Encrypt(nonce.AsSpan(0,NonceSize),plain.AsSpan(0,byteCount),cipher.AsSpan(0,byteCount),tag.AsSpan(0,TagSize),perChunkAad.AsSpan(0,PerChunkAadHashSize));

                    WriteUInt32BigEndian(lengths.AsSpan(0,IntSize),(UInt32)byteCount);
                    WriteUInt32BigEndian(lengths.AsSpan(IntSize,IntSize),(UInt32)byteCount);

                    await output.WriteAsync(lengths.AsMemory(0,ChunkLengthsBlockSize),cancel).ConfigureAwait(false);
                    await output.WriteAsync(cipher.AsMemory(0,byteCount),cancel).ConfigureAwait(false);
                    await output.WriteAsync(tagLength.AsMemory(),cancel).ConfigureAwait(false);
                    await output.WriteAsync(tag.AsMemory(0,TagSize),cancel).ConfigureAwait(false);

                    totalWritten += (UInt64)byteCount; chunkIndex++;
                }
            ...
            finally
            {
                ...
                ZeroMemory(tagLength);
            }
        }
        catch ( CryptographicException _ ) { KusDepotLog.Trace(_,EncryptUtf16AsyncFail); return false; }

        catch ( Exception _ ) { KusDepotLog.Error(_,EncryptUtf16AsyncFail); if(NoExceptions) { return false; } throw; }
