...
public class ToolWebHostBuilder : ToolBuilder , IToolWebHostBuilder
{
    ...
    public IToolWebHost BuildWebHost(WebApplication? app = null)
    {
        try
        {
            ...
            Guid i = Guid.NewGuid(); IToolHostLifetime? l; if(Console) { l = new ToolHostConsoleLifetime(i); } else { l = new ToolHostLifetime(i); }
            ...
            InitializeToolBuilderContext(); InitializeConfiguration(); InitializeServiceProvider(); ContainerBuilder!.RegisterInstance(l).AsImplementedInterfaces(); ToolBuilt = true;

            WebApplication? a; if(app is null) { a = Builder.Build(); if(configure is not null) { configure(a); } } else { a = app; }

            var _ = new ToolWebHost(accessmanager:AccessManager,commands:Commands!.ToArgument(),container:ContainerBuilder!.Build(),configuration:Context!.Configuration,urls:a.Urls,webapplication:a,lifeid:i,lifetime:l,logger:new LoggerConfiguration().WriteTo.ILogger(a.Logger).CreateLogger());
        ...
        catch ( Exception _ ) { Log.Error(_,BuilderFail); return new ToolWebHost(); }
    }
    ...
    public IToolWebHost BuildWebHost<TToolWebHost>(WebApplication? app = null) where TToolWebHost : notnull , IToolWebHost , new()
    {
        try
        {
            ...
            try { _ = (IToolWebHost?)Activator.CreateInstance(typeof(TToolWebHost),AccessManager,null,null,Commands!.ToArgument(),Context!.Configuration,ContainerBuilder!.Build(),null,i,l,null,a.Urls,a); }

            catch { _ = new ToolWebHost(accessmanager:AccessManager,commands:Commands!.ToArgument(),container:ContainerBuilder!.Build(),configuration:Context!.Configuration,urls:a.Urls,webapplication:a,lifeid:i,lifetime:l,logger:new LoggerConfiguration().WriteTo.ILogger(a.Logger).CreateLogger()); }

            if(Console) { var cl = l as ToolHostConsoleLifetime; cl!.ToolHost = _; cl.ManagerKey = _!.CreateManagementKey("Lifetime") as ManagerKey; } if(Connect) { c!.ToolWebHost = _; }

            if(Start) { ConfigureToolWebHost(StartAction); } if(seal) { ConfigureToolWebHost(SealAction); } configuretool(_!); configuretoolwebhost(_!); return _!;
        }
        catch ( Exception _ ) { Log.Error(_,BuilderFail); return new TToolWebHost(); }
    }
    ...
    public IToolWebHostBuilder UseSlimBuilder(WebApplicationOptions? options = null)
    {
        ...
        catch ( Exception _ ) { Log.Error(_,BuilderUseSlimBuilderFail); return this; }
    }
    ...
    public IToolWebHostBuilder UseBuilderOptions(WebApplicationOptions? options = null)
    {
        ...
        catch ( Exception _ ) { Log.Error(_,BuilderUseBuilderOptionsFail); return this; }
    }
    ...
    public IToolWebHostBuilder UseMcpServer(ServerCapabilities? capabilities = null , String? instructions = null)
    {
        ...
        catch ( Exception _ ) { Log.Error(_,BuilderUseMcpServerFail); return this; }
    }
    ...
    protected void configuretoolwebhost(IToolWebHost toolwebhost)
    {
        ...
        catch ( Exception _ ) { Log.Error(_,BuilderConfigureToolWebHostFail); }
    }
    ...
    public IToolWebHostBuilder ConfigureToolWebHost(Action<ToolBuilderContext,IToolWebHost> action)
    {
        ...
        catch ( Exception _ ) { Log.Error(_,BuilderConfigureToolWebHostFail); return this; }
    }
