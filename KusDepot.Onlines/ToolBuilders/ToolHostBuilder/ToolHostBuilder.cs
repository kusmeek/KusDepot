...
public class ToolHostBuilder : ToolBuilder , IToolHostBuilder
{
    ...
    public IToolHost BuildHost()
    {
        try
        {
            ...
            var _ = new ToolHost(accessmanager:AccessManager,commands:Commands.ToArgument(),configuration:Context!.Configuration,container:ContainerBuilder!.Build(),lifeid:i,lifetime:l);
        ...
        catch ( Exception _ ) { Log.Error(_,BuilderFail); return new ToolHost(); }
    }
    ...
    public IToolHost BuildHost<TToolHost>() where TToolHost : notnull , IToolHost , new()
    {
        try
        {
            ...
            IToolHostLifetime? l; Guid i = Guid.NewGuid(); if(Console) { l = new ToolHostConsoleLifetime(i); } else { l = new ToolHostLifetime(i); } IToolHost? _ = default;

            InitializeToolBuilderContext(); InitializeConfiguration(); InitializeServiceProvider(); ContainerBuilder!.RegisterInstance(l).AsImplementedInterfaces(); ToolBuilt = true;

            try { _ = (IToolHost?)Activator.CreateInstance(typeof(TToolHost),AccessManager,null,null,Commands.ToArgument(),Context!.Configuration,ContainerBuilder!.Build(),null,i,l,null); }

            catch { _ = new ToolHost(accessmanager:AccessManager,commands:Commands.ToArgument(),configuration:Context!.Configuration,container:ContainerBuilder!.Build(),lifeid:i,lifetime:l); }

            if(Console) { var cl = l as ToolHostConsoleLifetime; cl!.ToolHost = _; cl.ManagerKey = _!.CreateManagementKey("Lifetime") as ManagerKey; }

            if(Start) { ConfigureToolHost(StartAction); } if(seal) { ConfigureToolHost(SealAction); } configuretool(_!); configuretoolhost(_!); return _!;
        }
        catch ( Exception _ ) { Log.Error(_,BuilderFail); return new TToolHost(); }
    }
    ...
    protected void configuretoolhost(IToolHost toolhost)
    {
        ...
        catch ( Exception _ ) { Log.Error(_,BuilderConfigureToolHostFail); }
    }
    ...
    public IToolHostBuilder ConfigureToolHost(Action<ToolBuilderContext,IToolHost> action)
    {
        ...
        catch ( Exception _ ) { Log.Error(_,BuilderConfigureToolHostFail); return this; }
    }
