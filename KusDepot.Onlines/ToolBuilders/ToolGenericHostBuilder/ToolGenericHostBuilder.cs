...
public class ToolGenericHostBuilder : ToolBuilder , IToolGenericHostBuilder
{
    ...
    public IToolGenericHost BuildGenericHost(IHost? host = null)
    {
        try
        {
            ...
            if(Console) { l = new ToolHostConsoleLifetime(i); } else { l = new ToolHostLifetime(i); } this.ConfigureServices((c,s) => s.AddSingletonWithInterfaces(l!));
            ...
            var _ = new ToolGenericHost(accessmanager:AccessManager,services:BuildServiceProvider(),commands:Commands,configuration:Context!.Configuration,host:(host ?? h),lifeid:i,lifetime:l,logger:(host ?? h)?.Services.GetService<ILoggerFactory>());
    ...
    public IToolGenericHost BuildGenericHost<TToolGenericHost>(IHost? host = null) where TToolGenericHost : notnull , IToolGenericHost , new()
    {
        try
        {
            ...
            IToolHostLifetime? l; if(Console) { l = new ToolHostConsoleLifetime(i); } else { l = new ToolHostLifetime(i); } this.ConfigureServices((c,s) => s.AddSingletonWithInterfaces(l!));
            ...
            InitializeToolBuilderContext(); InitializeConfiguration(); ToolBuilt = true; IToolGenericHost? _ = default;

            try { _ = (IToolGenericHost?)Activator.CreateInstance(typeof(TToolGenericHost),AccessManager,null,null,BuildServiceProvider(),Commands,Context!.Configuration,host ?? h,i,l,(host ?? h)?.Services.GetService<ILoggerFactory>()); }

            catch { _ = new ToolGenericHost(accessmanager:AccessManager,commands:Commands,services:BuildServiceProvider(),configuration:Context!.Configuration,host:host ?? h,lifeid:i,lifetime:l,logger:(host ?? h)?.Services.GetService<ILoggerFactory>()); }
    ...
    public IToolGenericHostBuilder UseMcpServerDefaults(Assembly? toolassembly = null)
    {
        ...
        catch ( Exception _ ) { KusDepotLog.Error(_,BuilderUseMcpServerDefaultsFail); return this; }
    }
    ...
    public IToolGenericHostBuilder UseOrleans(Action<ISiloBuilder> configureorleans)
    {
        ...
        catch ( Exception _ ) { KusDepotLog.Error(_,BuilderUseOrleansFail); return this; }
    }
