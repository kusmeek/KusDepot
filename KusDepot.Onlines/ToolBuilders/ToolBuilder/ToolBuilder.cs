...
public partial class ToolBuilder : IToolBuilder
{
    ...
    public ToolBuilder() { try { this.ToolBuilt = false; } catch ( Exception _ ) { Log.Error(_,BuilderConstructorFail); } }
    ...
    protected ToolServiceProvider BuildServiceProvider()
    {
        try
        {
            ...
            ServiceCollection s = new ServiceCollection(); s.AddSingleton<IConfiguration>(this.Context!.Configuration!);
        ...
        catch ( Exception _ ) { Log.Error(_,BuilderBuildServiceProviderFail); throw; }
    }
    ...
    {
        try { return logger is not null ? this.ConfigureServices((x,s) => s.AddSingleton(logger)) : this; }
    ...
    public IToolBuilder UseServiceProvider(IServiceProvider? serviceprovider)
    {
        ...
        catch ( Exception _ ) { Log.Error(_,BuilderUseServiceProviderFail); return this; }
    }
