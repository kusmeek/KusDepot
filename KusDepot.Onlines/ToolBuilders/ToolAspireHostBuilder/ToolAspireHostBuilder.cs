...
public class ToolAspireHostBuilder : ToolBuilder , IToolAspireHostBuilder
{
    ...
    public IToolAspireHost BuildAspireHost(DistributedApplication? app = null)
    {
        try
        {
            ...
            DistributedApplication? a; if(app is null) { a = Builder.Build(); } else { a = app; }

            var _ = new ToolAspireHost(accessmanager:AccessManager,commands:Commands!.ToArgument(),container:ContainerBuilder!.Build(),configuration:Context!.Configuration,aspireapplication:a,lifeid:i,lifetime:l);
        ...
        catch ( Exception _ ) { Log.Error(_,BuilderFail); return new ToolAspireHost(); }
    }
    ...
    public IToolAspireHost BuildAspireHost<TToolAspireHost>(DistributedApplication? app = null) where TToolAspireHost : notnull , IToolAspireHost , new()
    {
        try
        {
            ...
            DistributedApplication? a; if(app is null) { a = Builder.Build(); } else { a = app; } IToolAspireHost? _ = default;

            try { _ = (IToolAspireHost?)Activator.CreateInstance(typeof(TToolAspireHost),AccessManager,null,null,Commands!.ToArgument(),Context!.Configuration,ContainerBuilder!.Build(),null,i,l,null,a); }

            catch { _ = new ToolAspireHost(accessmanager:AccessManager,commands:Commands!.ToArgument(),container:ContainerBuilder!.Build(),configuration:Context!.Configuration,aspireapplication:a,lifeid:i,lifetime:l); }

            if(Console) { var cl = l as ToolHostConsoleLifetime; cl!.ToolHost = _; cl.ManagerKey = _!.CreateManagementKey("Lifetime") as ManagerKey; } if(Connect) { c!.ToolAspireHost = _; }

            if(Start) { ConfigureToolAspireHost(StartAction); } if(seal) { ConfigureToolAspireHost(SealAction); } configuretool(_!); configuretoolaspirehost(_!); return _!;
        }
        catch ( Exception _ ) { Log.Error(_,BuilderFail); return new TToolAspireHost(); }
    }
    ...
    protected void configuretoolaspirehost(IToolAspireHost toolaspirehost)
    {
        ...
        catch ( Exception _ ) { Log.Error(_,BuilderConfigureToolAspireHostFail); }
    }
    ...
    public IToolAspireHostBuilder ConfigureToolAspireHost(Action<ToolBuilderContext,IToolAspireHost> action)
    {
        ...
        catch ( Exception _ ) { Log.Error(_,BuilderConfigureToolAspireHostFail); return this; }
    }
    ...
    public IToolAspireHostBuilder UseBuilderArguments(String[]? args = null)
    {
        ...
        catch ( Exception _ ) { Log.Error(_,BuilderUseBuilderArgumentsFail); return this; }
    }
